...
TEST(TestDefaultSessionResumptionStorage, TestSave)
{
    ...
    {
        EXPECT_TRUE(CHIP_NO_ERROR == chip::Crypto::DRBG_get_bytes(vectors[i].resumptionId.data(), vectors[i].resumptionId.size()));
    ...
    {
        EXPECT_TRUE(sessionStorage.Save(vectors[i].node, vectors[i].resumptionId, vectors[i].sharedSecret, vectors[i].cats) ==
                    CHIP_NO_ERROR);
    }
    ...
    // case should be modified to match.
    {
        ...
        EXPECT_TRUE(sessionStorage.Save(vectors[last].node, vectors[last].resumptionId, vectors[last].sharedSecret,
    ...
    for (auto & vector : vectors)
    {
        ...
        EXPECT_TRUE(sessionStorage.FindByScopedNodeId(vector.node, outResumptionId, outSharedSecret, outCats) == CHIP_NO_ERROR);
        EXPECT_TRUE(memcmp(vector.resumptionId.data(), outResumptionId.data(), vector.resumptionId.size()) == 0);
        EXPECT_TRUE(memcmp(vector.sharedSecret.ConstBytes(), outSharedSecret.ConstBytes(), vector.sharedSecret.Length()) == 0);
        EXPECT_TRUE(vector.cats.values[0] == outCats.values[0]);
        EXPECT_TRUE(vector.cats.values[1] == outCats.values[1]);
        EXPECT_TRUE(vector.cats.values[2] == outCats.values[2]);

        // Validate retrieval by resumption ID.
        EXPECT_TRUE(sessionStorage.FindByResumptionId(vector.resumptionId, outNode, outSharedSecret, outCats) == CHIP_NO_ERROR);
        EXPECT_TRUE(vector.node == outNode);
        EXPECT_TRUE(memcmp(vector.sharedSecret.Bytes(), outSharedSecret.ConstBytes(), vector.sharedSecret.Length()) == 0);
        EXPECT_TRUE(vector.cats.values[0] == outCats.values[0]);
        EXPECT_TRUE(vector.cats.values[1] == outCats.values[1]);
        EXPECT_TRUE(vector.cats.values[2] == outCats.values[2]);
    }
...
TEST(TestDefaultSessionResumptionStorage, TestInPlaceSave)
{
    ...
    for (size_t i = 0; i < ArraySize(nodes); ++i)
    {
        ...
        // Verify retrieval by node.
        EXPECT_TRUE(sessionStorage.FindByScopedNodeId(vectors[i].node, outResumptionId, outSharedSecret, outCats) == CHIP_NO_ERROR);
        EXPECT_TRUE(memcmp(vectors[i].resumptionId.data(), outResumptionId.data(), vectors[i].resumptionId.size()) == 0);
        EXPECT_TRUE(memcmp(vectors[i].sharedSecret.ConstBytes(), outSharedSecret.ConstBytes(), vectors[i].sharedSecret.Length()) ==
                    0);
        EXPECT_TRUE(vectors[i].cats.values[0] == outCats.values[0]);
        EXPECT_TRUE(vectors[i].cats.values[1] == outCats.values[1]);
        EXPECT_TRUE(vectors[i].cats.values[2] == outCats.values[2]);

        // Validate retrieval by resumption ID.
        EXPECT_TRUE(sessionStorage.FindByResumptionId(vectors[i].resumptionId, outNode, outSharedSecret, outCats) == CHIP_NO_ERROR);
        EXPECT_TRUE(vectors[i].node == outNode);
        EXPECT_TRUE(memcmp(vectors[i].sharedSecret.ConstBytes(), outSharedSecret.ConstBytes(), vectors[i].sharedSecret.Length()) ==
                    0);
        EXPECT_TRUE(vectors[i].cats.values[0] == outCats.values[0]);
        EXPECT_TRUE(vectors[i].cats.values[1] == outCats.values[1]);
    ...
    for (auto & vector : vectors)
    {
        EXPECT_TRUE(sessionStorage.Save(vector.node, vector.resumptionId, vector.sharedSecret, vector.cats) == CHIP_NO_ERROR);
    }
    ...
    {
        EXPECT_TRUE(sessionStorage.DeleteAll(node.GetFabricIndex()) == CHIP_NO_ERROR);
    }
    ...
    for (const auto & node : nodes)
    {
        ...
        EXPECT_TRUE(rv == CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
    }
...
TEST(TestDefaultSessionResumptionStorage, TestDelete)
{
    ...
    for (auto & vector : vectors)
    {
        EXPECT_TRUE(sessionStorage.Save(vector.node, vector.resumptionId, sharedSecret, chip::CATValues{}) == CHIP_NO_ERROR);
    }
    ...
    for (auto & vector : vectors)
    {
        ...
        chip::Crypto::P256ECDHDerivedSecret outSharedSecret;
        chip::CATValues outCats;
        EXPECT_TRUE(sessionStorage.Delete(vector.node) == CHIP_NO_ERROR);
        EXPECT_TRUE(sessionStorage.FindByScopedNodeId(vector.node, outResumptionId, outSharedSecret, outCats) != CHIP_NO_ERROR);
        EXPECT_TRUE(sessionStorage.FindByResumptionId(vector.resumptionId, outNode, outSharedSecret, outCats) != CHIP_NO_ERROR);
    }
...
TEST(TestDefaultSessionResumptionStorage, TestDeleteAll)
{
    ...
    for (auto & vector : vectors)
    {
        ...
        {
            EXPECT_TRUE(sessionStorage.Save(node.node, node.resumptionId, sharedSecret, chip::CATValues{}) == CHIP_NO_ERROR);
        }
    }
    ...
    for (const auto & vector : vectors)
    {
        ...
        {
            EXPECT_TRUE(sessionStorage.FindByScopedNodeId(node.node, outResumptionId, outSharedSecret, outCats) == CHIP_NO_ERROR);
        }
        // Delete fabric.
        EXPECT_TRUE(sessionStorage.DeleteAll(vector.fabricIndex) == CHIP_NO_ERROR);
