...
class TestStatusReport : public ::testing::Test
{
public:
...
TEST_F(TestStatusReport, TestStatusReport_NoData)
{
    ...
    EXPECT_TRUE(reportToParse.GetGeneralCode() == generalCode);
    EXPECT_TRUE(reportToParse.GetProtocolId() == protocolId);
    EXPECT_TRUE(reportToParse.GetProtocolCode() == protocolCode);

    const System::PacketBufferHandle & data = reportToParse.GetProtocolData();
    EXPECT_TRUE(data.IsNull());
}

TEST_F(TestStatusReport, TestStatusReport_WithData)
{
    ...
    EXPECT_TRUE(rcvData->DataLength() == dataLen);
    EXPECT_TRUE(!memcmp(rcvData->Start(), data, dataLen));
}
...
TEST_F(TestStatusReport, TestMakeBusyStatusReport)
{
    ...
    const System::PacketBufferHandle & rcvData = reportToParse.GetProtocolData();
    EXPECT_TRUE(!rcvData.IsNull());
    EXPECT_TRUE(rcvData->DataLength() == sizeof(minimumWaitTime));

    uint16_t readMinimumWaitTime = 0;
    Encoding::LittleEndian::Reader reader(rcvData->Start(), rcvData->DataLength());
    EXPECT_TRUE(CHIP_NO_ERROR == reader.Read16(&readMinimumWaitTime).StatusCode());
    EXPECT_TRUE(System::Clock::Milliseconds16(readMinimumWaitTime) == minimumWaitTime);
}
