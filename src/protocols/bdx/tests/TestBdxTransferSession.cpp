...
void VerifyBdxMessageToSend(const TransferSession::OutputEvent & outEvent, MessageType expected)
{
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kMsgToSend);
    EXPECT_TRUE(!outEvent.MsgData.IsNull());
    EXPECT_TRUE(outEvent.msgTypeData.ProtocolId == Protocols::BDX::Id);
    EXPECT_TRUE(outEvent.msgTypeData.MessageType == static_cast<uint8_t>(expected));
}
...
void VerifyStatusReport(const System::PacketBufferHandle & msg, StatusCode expectedCode)
{
    ...
    EXPECT_TRUE(report.GetGeneralCode() == SecureChannel::GeneralStatusCode::kFailure);
    EXPECT_TRUE(report.GetProtocolId() == Protocols::BDX::Id);
    EXPECT_TRUE(report.GetProtocolCode() == static_cast<uint16_t>(expectedCode));
}
...
void VerifyNoMoreOutput(TransferSession & transferSession)
{
    ...
    EXPECT_TRUE(event.EventType == TransferSession::OutputEventType::kNone);
}
                               ...
                               BitFlags<TransferControlFlags> & responderControlOpts, uint16_t responderMaxBlock)
{
    ...
    VerifyNoMoreOutput(responder);
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kInitReceived);
    EXPECT_TRUE(outEvent.transferInitData.TransferCtlFlags == initData.TransferCtlFlags);
    EXPECT_TRUE(outEvent.transferInitData.MaxBlockSize == initData.MaxBlockSize);
    EXPECT_TRUE(outEvent.transferInitData.StartOffset == initData.StartOffset);
    EXPECT_TRUE(outEvent.transferInitData.Length == initData.Length);
    EXPECT_TRUE(outEvent.transferInitData.FileDesignator != nullptr);
    EXPECT_TRUE(outEvent.transferInitData.FileDesLength == initData.FileDesLength);
    ...
    {
        EXPECT_TRUE(outEvent.transferInitData.MetadataLength == initData.MetadataLength);
                            ...
                            TransferSession::TransferInitData initData)
{
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kAcceptReceived);
    EXPECT_TRUE(outEvent.transferAcceptData.ControlMode == acceptData.ControlMode);
    EXPECT_TRUE(outEvent.transferAcceptData.MaxBlockSize == acceptData.MaxBlockSize);
    EXPECT_TRUE(outEvent.transferAcceptData.StartOffset == acceptData.StartOffset);
    EXPECT_TRUE(outEvent.transferAcceptData.Length == acceptData.Length);
    if (outEvent.transferAcceptData.Metadata != nullptr)
    {
        EXPECT_TRUE(outEvent.transferAcceptData.MetadataLength == acceptData.MetadataLength);
    ...
    EXPECT_TRUE(acceptReceiver.GetTransferBlockSize() <= initData.MaxBlockSize);
}
...
void SendAndVerifyQuery(TransferSession & queryReceiver, TransferSession & querySender, TransferSession::OutputEvent & outEvent)
{
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kQueryReceived);
                                 ...
                                 bool isEof, uint32_t inBlockCounter)
{
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kBlockReceived);
    EXPECT_TRUE(outEvent.blockdata.Data != nullptr);
    if (outEvent.EventType == TransferSession::OutputEventType::kBlockReceived && outEvent.blockdata.Data != nullptr)
    {
        EXPECT_TRUE(!memcmp(fakeBlockData, outEvent.blockdata.Data, outEvent.blockdata.Length));
        EXPECT_TRUE(outEvent.blockdata.BlockCounter == inBlockCounter);
    }
                           ...
                           bool expectEOF)
{
    ...
    EXPECT_TRUE(outEvent.EventType == expectedEventType);
    VerifyNoMoreOutput(ackReceiver);
}

class TestBdxTransferSession : public ::testing::Test
{
public:
...
TEST_F(TestBdxTransferSession, TestInitiatingReceiverReceiverDrive)
{
    ...
    EXPECT_TRUE(respondingSender.GetTransferBlockSize() == testSmallerBlockSize);
    EXPECT_TRUE(respondingSender.GetTransferBlockSize() == initiatingReceiver.GetTransferBlockSize());
    ...
    EXPECT_TRUE(outEvent.blockdata.IsEof == true);
...
TEST_F(TestBdxTransferSession, TestTimeout)
{
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kTransferTimeout);
}
...
TEST_F(TestBdxTransferSession, TestDuplicateBlockError)
{
    ...
    EXPECT_TRUE(eventWithBlock.EventType == TransferSession::OutputEventType::kMsgToSend);
    ...
    {
        initiatingReceiver.PollOutput(outEvent, kNoAdvanceTime);
        EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kInternalError);
        EXPECT_TRUE(outEvent.statusData.statusCode == StatusCode::kBadBlockCounter);
    }
    ...
    EXPECT_TRUE(outEvent.EventType == TransferSession::OutputEventType::kStatusReceived);
    EXPECT_TRUE(outEvent.statusData.statusCode == StatusCode::kBadBlockCounter);
