...
class TestGroupOperationalCredentials : public ::testing::Test
{
public:
...
TEST_F(TestGroupOperationalCredentials, TestDeriveGroupOperationalCredentials)
{
    ...
    for (const auto & testVector : theGroupKeySetTestVector)
    {
        ...
        EXPECT_TRUE(CHIP_NO_ERROR == Crypto::DeriveGroupOperationalCredentials(epochKey, kCompressedFabricId1, opCreds));

        EXPECT_TRUE(opCreds.hash == testVector.groupKeys->hash);
        EXPECT_TRUE(0 == memcmp(opCreds.encryption_key, testVector.groupKeys->encryption_key, KEY_LENGTH));
        EXPECT_TRUE(0 == memcmp(opCreds.privacy_key, testVector.groupKeys->privacy_key, KEY_LENGTH));
    }
}
} // namespace