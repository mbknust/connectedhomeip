...
class [[maybe_unused]] HeapChecker
{
public:
    ...
    ~HeapChecker()
    {
        ...
        if (bytesUsed != mHeapBytesUsed)
        {
            ...
            EXPECT_TRUE(bytesUsed == mHeapBytesUsed);
        }
    }
...
#else
class [[maybe_unused]] HeapChecker{ public : };
#endif
...
class TestCryptoPAL : public ::testing::Test
{
public:
...
TEST_F(TestCryptoPAL, TestAES_CTR_128CryptTestVectors)
{
    ...
    EXPECT_TRUE(numOfTestsRan > 0);
}

TEST_F(TestCryptoPAL, TestAES_CCM_128EncryptTestVectors)
{
    HeapChecker heapChecker;
...
TEST_F(TestCryptoPAL, TestAES_CCM_128DecryptTestVectors)
{
    HeapChecker heapChecker;
    int numOfTestVectors = ArraySize(ccm_128_test_vectors);
    int numOfTestsRan    = 0;
...
TEST_F(TestCryptoPAL, TestAES_CCM_128EncryptInvalidNonceLen)
{
    HeapChecker heapChecker;
    int numOfTestVectors = ArraySize(ccm_128_test_vectors);
    int numOfTestsRan    = 0;
...
TEST_F(TestCryptoPAL, TestAES_CCM_128EncryptInvalidTagLen)
{
    HeapChecker heapChecker;
    int numOfTestVectors = ArraySize(ccm_128_test_vectors);
    int numOfTestsRan    = 0;
...
TEST_F(TestCryptoPAL, TestAES_CCM_128DecryptInvalidNonceLen)
{
    HeapChecker heapChecker;
    int numOfTestVectors = ArraySize(ccm_128_test_vectors);
    int numOfTestsRan    = 0;
...
TEST_F(TestCryptoPAL, TestSensitiveDataBuffer)
{
    HeapChecker heapChecker;
    ...
    // Test initial value
    Buffer buffer;
    EXPECT_TRUE(buffer.ConstBytes() == (const uint8_t *) buffer.Bytes());
    EXPECT_TRUE(buffer.Length() == 0);
    ...
    EXPECT_TRUE(buffer.ConstBytes() == (const uint8_t *) buffer.Bytes());
    EXPECT_TRUE(buffer.ConstBytes() == buffer.Span().data());
    EXPECT_TRUE(buffer.Length() == kLength);
    EXPECT_TRUE(buffer.Length() == buffer.Span().size());

    // Test sanitization of entire buffer (even though length < capacity)
    const void * bufferStorage = buffer.ConstBytes();
    buffer.~Buffer();
    EXPECT_TRUE(memcmp(bufferStorage, kAllZeros, kCapacity) == 0);
    EXPECT_TRUE(memcmp(bufferStorage, testVector, kCapacity));
}

TEST_F(TestCryptoPAL, TestSensitiveDataFixedBuffer)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(buffer.ConstBytes() == (const uint8_t *) buffer.Bytes());
    EXPECT_TRUE(buffer.ConstBytes() == buffer.Span().data());
    EXPECT_TRUE(memcmp(buffer.ConstBytes(), testVector, kCapacity) == 0);
...
TEST_F(TestCryptoPAL, TestAsn1Conversions)
{
    HeapChecker heapChecker;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(status == CHIP_NO_ERROR);
        EXPECT_TRUE(out_raw_sig_span.size() == vector->raw_version_length);
        EXPECT_TRUE((memcmp(out_raw_sig_span.data(), vector->raw_version, vector->raw_version_length) == 0));
        ...
        status = EcdsaRawSignatureToAsn1(vector->fe_length_bytes, ByteSpan{ vector->raw_version, vector->raw_version_length },
                                         out_der_sig_span);
        EXPECT_TRUE(status == CHIP_NO_ERROR);
        EXPECT_TRUE(out_der_sig_span.size() <= out_der_sig_allocated_size);
        EXPECT_TRUE(out_der_sig_span.size() == vector->der_version_length);
        EXPECT_TRUE((memcmp(out_der_sig_span.data(), vector->der_version, vector->der_version_length) == 0));
    }
}

TEST_F(TestCryptoPAL, TestRawIntegerToDerValidCases)
{
    HeapChecker heapChecker;
    ...
    for (int testIdx = 0; testIdx < numOfTestCases; testIdx++)
    {
        ...
        // Cover case with tag/length
        {
            ...
            EXPECT_TRUE(status == CHIP_NO_ERROR);
            EXPECT_TRUE(out_der_integer.size() == v.expected_size);
            ...
            EXPECT_TRUE(status == CHIP_ERROR_BUFFER_TOO_SMALL);
        }
        ...
        // Cover case without tag/length
        {
            ...
            EXPECT_TRUE(status == CHIP_NO_ERROR);
            EXPECT_TRUE(out_der_integer.size() == v.expected_without_tag_size);
            EXPECT_TRUE(out_der_integer.data_equal(ByteSpan(v.expected_without_tag, v.expected_without_tag_size)));
        }
    }
}

TEST_F(TestCryptoPAL, TestRawIntegerToDerInvalidCases)
{
    HeapChecker heapChecker;
    ...
    for (const ErrorCase & v : error_cases)
    {
        ...
        if (status != v.expected_status)
        {
            ...
            EXPECT_TRUE(v.expected_status == status);
        }
        ++case_idx;
    }
}

TEST_F(TestCryptoPAL, TestReadDerLengthValidCases)
{
    ...
    for (const SuccessCase & v : cases)
    {
        ...
        if ((status != CHIP_NO_ERROR) || (v.expected_length != output_length))
        {
            ...
            ASSERT_EQ(output_length, v.expected_length);
            ASSERT_EQ(status, CHIP_NO_ERROR);
        }
        ++case_idx;
    }
}

TEST_F(TestCryptoPAL, TestReadDerLengthInvalidCases)
{
    ...
    for (const ErrorCase & v : error_cases)
    {
        ...
        if (status != v.expected_status)
        {
            ...
            ASSERT_EQ(v.expected_status, status);
        }
        ++case_idx;
    }
}

TEST_F(TestCryptoPAL, TestHash_SHA256)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(numOfTestsExecuted == ArraySize(hash_sha256_test_vectors));
}

TEST_F(TestCryptoPAL, TestHash_SHA256_Stream)
{
    HeapChecker heapChecker;
    int numOfTestCases     = ArraySize(hash_sha256_test_vectors);
    int numOfTestsExecuted = 0;
    ...
    for (numOfTestsExecuted = 0; numOfTestsExecuted < numOfTestCases; numOfTestsExecuted++)
    {
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(out_span.size() == kSHA256_Hash_Length);
    ...
    for (size_t block1_size = 1; block1_size <= kSHA256_Hash_Length; block1_size++)
    {
        for (size_t block2_size = 1; block2_size <= kSHA256_Hash_Length; block2_size++)
        {
            ...
            Hash_SHA256_stream sha256;
            EXPECT_TRUE(sha256.Begin() == CHIP_NO_ERROR);

            // Compute partial digest after first block
            EXPECT_TRUE(sha256.AddData(ByteSpan{ &source_buf[0], block1_size }) == CHIP_NO_ERROR);
            EXPECT_TRUE(sha256.GetDigest(partial_digest_span1) == CHIP_NO_ERROR);
            EXPECT_TRUE(partial_digest_span1.size() == kSHA256_Hash_Length);

            // Validate partial digest matches expectations
            Hash_SHA256(&source_buf[0], block1_size, &partial_digest_ref[0]);
            EXPECT_TRUE(0 == memcmp(partial_digest_span1.data(), partial_digest_ref, partial_digest_span1.size()));

            // Compute partial digest and total digest after second block
            EXPECT_TRUE(sha256.AddData(ByteSpan{ &source_buf[block1_size], block2_size }) == CHIP_NO_ERROR);

            EXPECT_TRUE(sha256.GetDigest(partial_digest_span2) == CHIP_NO_ERROR);
            EXPECT_TRUE(partial_digest_span2.size() == kSHA256_Hash_Length);

            EXPECT_TRUE(sha256.Finish(total_digest_span) == CHIP_NO_ERROR);
            EXPECT_TRUE(total_digest_span.size() == kSHA256_Hash_Length);

            // Validate second partial digest matches final digest
            Hash_SHA256(&source_buf[0], block1_size + block2_size, &total_digest_ref[0]);
            EXPECT_TRUE(0 == memcmp(partial_digest_span2.data(), total_digest_ref, partial_digest_span2.size()));
            EXPECT_TRUE(0 == memcmp(total_digest_span.data(), total_digest_ref, total_digest_span.size()));
        }
    }
    ...
    // Validate error cases
    {
        ...
        EXPECT_TRUE(sha256.AddData(ByteSpan{ source_buf2 }) == CHIP_NO_ERROR);

        // Check that error behavior works on buffer too small
        EXPECT_TRUE(sha256.GetDigest(digest_span_too_small) == CHIP_ERROR_BUFFER_TOO_SMALL);
        EXPECT_TRUE(sha256.Finish(digest_span_too_small) == CHIP_ERROR_BUFFER_TOO_SMALL);

        // Check that both GetDigest/Finish can still work after error.
        EXPECT_TRUE(sha256.GetDigest(digest_span_ok) == CHIP_NO_ERROR);
        EXPECT_TRUE(0 == memcmp(digest_span_ok.data(), digest_buf_ref, digest_span_ok.size()));

        memset(digest_buf_ok, 0, sizeof(digest_buf_ok));

        EXPECT_TRUE(sha256.Finish(digest_span_ok) == CHIP_NO_ERROR);
        EXPECT_TRUE(0 == memcmp(digest_span_ok.data(), digest_buf_ref, digest_span_ok.size()));
    }
}

TEST_F(TestCryptoPAL, TestHMAC_SHA256)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(numOfTestsExecuted == numOfTestCases);
}

TEST_F(TestCryptoPAL, TestHKDF_SHA256)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(numOfTestsExecuted == 3);
}

TEST_F(TestCryptoPAL, TestDRBG_InvalidInputs)
{
    HeapChecker heapChecker;
    CHIP_ERROR error = CHIP_NO_ERROR;
    error            = DRBG_get_bytes(nullptr, 10);
    EXPECT_TRUE(error == CHIP_ERROR_INVALID_ARGUMENT);
    error = CHIP_NO_ERROR;
    uint8_t buffer[5];
...
TEST_F(TestCryptoPAL, TestDRBG_Output)
{
    ...
    EXPECT_TRUE(error == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(out_buf, orig_buf, sizeof(out_buf)) != 0);
}

TEST_F(TestCryptoPAL, TestECDSA_Signing_SHA256_Msg)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(validation_error == CHIP_NO_ERROR);
}

TEST_F(TestCryptoPAL, TestECDSA_Signing_SHA256_Hash)
{
    HeapChecker heapChecker;
    ...
    for (int i = 0; i < kNumSigningIterations; ++i)
    {
        ...
        EXPECT_TRUE(Hash_SHA256(&msg[0], msg_length, &hash[0]) == CHIP_NO_ERROR);
...
TEST_F(TestCryptoPAL, TestECDSA_ValidationFailsDifferentMessage)
{
    HeapChecker heapChecker;
    const char * msg  = "Hello World!";
    size_t msg_length = strlen(msg);
    ...
    EXPECT_TRUE(validation_error == CHIP_ERROR_INVALID_SIGNATURE);
}

TEST_F(TestCryptoPAL, TestECDSA_ValidationFailIncorrectMsgSignature)
{
    HeapChecker heapChecker;
    const char * msg  = "Hello World!";
    size_t msg_length = strlen(msg);
...
TEST_F(TestCryptoPAL, TestECDSA_ValidationFailIncorrectHashSignature)
{
    HeapChecker heapChecker;
    const uint8_t msg[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
                            0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F };
...
TEST_F(TestCryptoPAL, TestECDSA_SigningMsgInvalidParams)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(signing_error == CHIP_ERROR_INVALID_ARGUMENT);
...
TEST_F(TestCryptoPAL, TestECDSA_ValidationMsgInvalidParam)
{
    HeapChecker heapChecker;
    const char * msg  = "Hello World!";
    size_t msg_length = strlen(msg);
    ...
    EXPECT_TRUE(validation_error == CHIP_ERROR_INVALID_ARGUMENT);
...
TEST_F(TestCryptoPAL, TestECDSA_ValidationHashInvalidParam)
{
    HeapChecker heapChecker;
    const uint8_t msg[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
                            0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F };
...
TEST_F(TestCryptoPAL, TestECDH_EstablishSecret)
{
    HeapChecker heapChecker;
    Test_P256Keypair keypair1;
    EXPECT_TRUE(keypair1.Initialize(ECPKeyTarget::ECDH) == CHIP_NO_ERROR);

    Test_P256Keypair keypair2;
    EXPECT_TRUE(keypair2.Initialize(ECPKeyTarget::ECDH) == CHIP_NO_ERROR);
    ...
    CHIP_ERROR error = CHIP_NO_ERROR;
    EXPECT_TRUE(memcmp(out_secret1.ConstBytes(), out_secret2.ConstBytes(), out_secret1.Capacity()) !=
...
TEST_F(TestCryptoPAL, TestAddEntropySources)
{
    HeapChecker heapChecker;
    CHIP_ERROR error = add_entropy_source(test_entropy_source, nullptr, 10);
    EXPECT_TRUE(error == CHIP_NO_ERROR);
    uint8_t buffer[5];
    EXPECT_TRUE(DRBG_get_bytes(buffer, sizeof(buffer)) == CHIP_NO_ERROR);
}
#endif
...
TEST_F(TestCryptoPAL, TestAddEntropySources)
{
    ...
    EXPECT_TRUE(gs_test_entropy_source_called > test_entropy_source_call_count);
}
#endif
...
TEST_F(TestCryptoPAL, TestPBKDF2_SHA256_TestVectors)
{
    HeapChecker heapChecker;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        if (vector->plen > 0)
        {
            ...
            if (vector->result == CHIP_NO_ERROR)
            {
                EXPECT_TRUE(memcmp(out_key.Get(), vector->key, vector->key_len) == 0);
            }
        }
    }
    EXPECT_TRUE(numOfTestsRan > 0);
...
TEST_F(TestCryptoPAL, TestP256_Keygen)
{
    HeapChecker heapChecker;
    P256Keypair keypair;
    EXPECT_TRUE(keypair.Initialize(ECPKeyTarget::ECDSA) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(keypair.ECDSA_sign_msg(test_msg, msglen, test_sig) == CHIP_NO_ERROR);
    EXPECT_TRUE(keypair.Pubkey().ECDSA_validate_msg_signature(test_msg, msglen, test_sig) == CHIP_NO_ERROR);
}

TEST_F(TestCryptoPAL, TestCSR_Verify)
{
    ...
    // First case: there is trailing garbage in the CSR
    {
        ...
        EXPECT_TRUE(err == CHIP_ERROR_UNSUPPORTED_CERT_FORMAT);
    }
...
TEST_F(TestCryptoPAL, TestCSR_GenDirect)
{
    ...
    EXPECT_TRUE(GenerateCertificateSigningRequest(&keypair, csrSpanTooSmall) == CHIP_ERROR_BUFFER_TOO_SMALL);

    // Validate case of null keypair
    EXPECT_TRUE(GenerateCertificateSigningRequest(nullptr, csrSpan) == CHIP_ERROR_INVALID_ARGUMENT);

    // Validate normal case
    ClearSecretData(csrBuf);
    EXPECT_TRUE(GenerateCertificateSigningRequest(&keypair, csrSpan) == CHIP_NO_ERROR);
    ...
    {
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        EXPECT_TRUE(pubkey.Length() == kP256_PublicKey_Length);
        EXPECT_TRUE(memcmp(pubkey.ConstBytes(), keypair.Pubkey().ConstBytes(), pubkey.Length()) == 0);
        ...
        EXPECT_TRUE(VerifyCertificateSigningRequest(csrSpan.data(), csrSpan.size(), pubkey) != CHIP_NO_ERROR);
    }
    else
    {
        ChipLogError(Crypto, "The current platform does not support CSR parsing.");
...
TEST_F(TestCryptoPAL, TestCSR_GenByKeypair)
{
    HeapChecker heapChecker;
    ...
    Test_P256Keypair keypair;
    EXPECT_TRUE(keypair.Initialize(ECPKeyTarget::ECDSA) == CHIP_NO_ERROR);
    EXPECT_TRUE(keypair.NewCertificateSigningRequest(csr, length) == CHIP_NO_ERROR);
    ...
    if (err != CHIP_ERROR_UNSUPPORTED_CHIP_FEATURE)
    {
        ...
        EXPECT_TRUE(VerifyCertificateSigningRequest(csr, length, pubkey) != CHIP_NO_ERROR);
    }
    else
    {
        ChipLogError(Crypto, "The current platform does not support CSR parsing.");
...
TEST_F(TestCryptoPAL, TestKeypair_Serialize)
{
    HeapChecker heapChecker;
    Test_P256Keypair keypair;
    ...
    EXPECT_TRUE(keypair.Serialize(serialized) == CHIP_NO_ERROR);

    Test_P256Keypair keypair_dup;
    EXPECT_TRUE(keypair_dup.Deserialize(serialized) == CHIP_NO_ERROR);
    ...
    P256ECDSASignature test_sig;
    EXPECT_TRUE(keypair.ECDSA_sign_msg(test_msg, msglen, test_sig) == CHIP_NO_ERROR);
    EXPECT_TRUE(keypair_dup.Pubkey().ECDSA_validate_msg_signature(test_msg, msglen, test_sig) == CHIP_NO_ERROR);

    EXPECT_TRUE(keypair_dup.ECDSA_sign_msg(test_msg, msglen, test_sig) == CHIP_NO_ERROR);
    EXPECT_TRUE(keypair.Pubkey().ECDSA_validate_msg_signature(test_msg, msglen, test_sig) == CHIP_NO_ERROR);
}

TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_FEMul)
{
    HeapChecker heapChecker;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(memcmp(fe_out, vector->fe_out, vector->fe_out_len) == 0);
        numOfTestsRan += 1;
    }
    EXPECT_TRUE(numOfTestsRan > 0);
    EXPECT_TRUE(numOfTestsRan == numOfTestVectors);
}

TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_FELoadWrite)
{
    HeapChecker heapChecker;
    uint8_t fe_out[kMAX_FE_Length];
...
TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_Mac)
{
    HeapChecker heapChecker;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(memcmp(mac_span.data(), vector->output, vector->output_len) == 0);
...
TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_PointMul)
{
    HeapChecker heapChecker;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(memcmp(output, vector->out_point, vector->out_point_len) == 0);
...
TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_PointMulAdd)
{
    HeapChecker heapChecker;
    uint8_t output[kMAX_Point_Length];
    size_t out_len = sizeof(output);
...
TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_PointLoadWrite)
{
    HeapChecker heapChecker;
    uint8_t output[kMAX_Point_Length];
    size_t out_len = sizeof(output);
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(memcmp(output, vector->point, vector->point_len) == 0);
...
TEST_F(TestCryptoPAL, TestSPAKE2P_spake2p_PointIsValid)
{
    HeapChecker heapChecker;
...
TEST_F(TestCryptoPAL, TestSPAKE2P_RFC)
{
    HeapChecker heapChecker;
    CHIP_ERROR error = CHIP_NO_ERROR;
    ...
    for (int vectorIndex = 0; vectorIndex < numOfTestVectors; vectorIndex++)
    {
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(X_len == vector->X_len);
        EXPECT_TRUE(memcmp(X, vector->X, vector->X_len) == 0);
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(L_len == vector->L_len);
        EXPECT_TRUE(memcmp(L, vector->L, vector->L_len) == 0);
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(Y_len == vector->Y_len);
        EXPECT_TRUE(memcmp(Y, vector->Y, vector->Y_len) == 0);
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(Vverifier_len == vector->MAC_KcB_len);
        EXPECT_TRUE(memcmp(Vverifier, vector->MAC_KcB, vector->MAC_KcB_len) == 0);

        error = Verifier.PointWrite(Verifier.Z, Z, kP256_Point_Length);
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(memcmp(Z, vector->Z, vector->Z_len) == 0);

        error = Verifier.PointWrite(Verifier.V, V, kP256_Point_Length);
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(memcmp(V, vector->V, vector->V_len) == 0);
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(Pverifier_len == vector->MAC_KcA_len);
        EXPECT_TRUE(memcmp(Pverifier, vector->MAC_KcA, vector->MAC_KcA_len) == 0);
        ...
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(PKe_len == vector->Ke_len);
        EXPECT_TRUE(memcmp(PKe, vector->Ke, vector->Ke_len) == 0);

        VKe_len = sizeof(VKe);
        error   = Verifier.GetKeys(VKe, &VKe_len);
        EXPECT_TRUE(error == CHIP_NO_ERROR);
        EXPECT_TRUE(VKe_len == vector->Ke_len);
        EXPECT_TRUE(memcmp(VKe, vector->Ke, vector->Ke_len) == 0);
...
TEST_F(TestCryptoPAL, TestCompressedFabricIdentifier)
{
    HeapChecker heapChecker;
    ...
    EXPECT_TRUE(error == CHIP_NO_ERROR);
    EXPECT_TRUE(compressed_fabric_id_span.size() == kCompressedFabricIdentifierSize);
    EXPECT_TRUE(
        0 ==
    ...
    EXPECT_TRUE(error == CHIP_NO_ERROR);
    EXPECT_TRUE(compressed_fabric_id_large_span.size() == kCompressedFabricIdentifierSize);
    EXPECT_TRUE(0 ==
    ...
    EXPECT_TRUE(error == CHIP_ERROR_BUFFER_TOO_SMALL);
    ...
    EXPECT_TRUE(error == CHIP_NO_ERROR);
    EXPECT_TRUE(compressed_fabric_id_int == kExpectedCompressedFabricIdentifierInt);
...
TEST_F(TestCryptoPAL, TestPubkey_x509Extraction)
{
    ...
    for (size_t i = 0; i < gNumTestCerts; i++)
    {
        ...
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        EXPECT_TRUE(publicKey.Length() == pubkeySpan.size());
        EXPECT_TRUE(memcmp(publicKey.ConstBytes(), pubkeySpan.data(), pubkeySpan.size()) == 0);
    }
}

TEST_F(TestCryptoPAL, TestX509_VerifyAttestationCertificateFormat)
{
    using namespace TestCerts;
    ...
    for (auto & testCase : sValidationTestCases)
    {
        ...
        EXPECT_TRUE(err == testCase.expectedError);
        ++case_idx;
    }
}

TEST_F(TestCryptoPAL, TestX509_CertChainValidation)
{
    using namespace TestCerts;
    ...
    for (auto & testCase : sValidationTestCases)
    {
        ...
        EXPECT_TRUE(err == testCase.expectedError);
        EXPECT_TRUE(chainValidationResult == testCase.expectedValResult);
    }
}

TEST_F(TestCryptoPAL, TestX509_IssuingTimestampValidation)
{
    using namespace TestCerts;
...
TEST_F(TestCryptoPAL, TestSKID_x509Extraction)
{
    using namespace TestCerts;
...
TEST_F(TestCryptoPAL, TestAKID_x509Extraction)
{
    using namespace TestCerts;
...
TEST_F(TestCryptoPAL, TestCDPExtension_x509Extraction)
{
    using namespace TestCerts;
    ...
    for (auto & testCase : sCDPTestCases)
    {
        ...
        EXPECT_TRUE(err == testCase.mExpectedError);
        if (testCase.mExpectedError == CHIP_NO_ERROR)
        {
            EXPECT_TRUE(cdp.size() == testCase.mExpectedResult.size());
            EXPECT_TRUE(cdp.data_equal(testCase.mExpectedResult));
        }
    }
}

TEST_F(TestCryptoPAL, TestCDPCRLIssuerExtension_x509Extraction)
{
    using namespace TestCerts;
...
TEST_F(TestCryptoPAL, TestSerialNumber_x509Extraction)
{
    using namespace TestCerts;
...
TEST_F(TestCryptoPAL, TestSubject_x509Extraction)
{
    using namespace TestCerts;
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Root01.AddAttribute_MatterRCACId(0xCACACACA00000001));
    ChipDN subjectDN_ICA01;
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_ICA01.AddAttribute_MatterICACId(0xCACACACA00000003));
    ChipDN subjectDN_Node02_02;
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_02.AddAttribute_MatterNodeId(0xDEDEDEDE00020002));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_02.AddAttribute_MatterFabricId(0xFAB000000000001D));
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_04.AddAttribute_MatterCASEAuthTag(0xABCE1002));
    EXPECT_TRUE(CHIP_NO_ERROR ==
                subjectDN_Node02_04.AddAttribute_CommonName(chip::CharSpan::fromCharString("TestCert02_04"), false));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_04.AddAttribute_MatterFabricId(0xFAB000000000001D));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_04.AddAttribute_MatterCASEAuthTag(0xABCD0003));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_04.AddAttribute_MatterNodeId(0xDEDEDEDE00020004));
    ChipDN subjectDN_Node02_08;
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_08.AddAttribute_MatterCASEAuthTag(0xABCF00A0));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_08.AddAttribute_MatterNodeId(0xDEDEDEDE00020008));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_08.AddAttribute_MatterCASEAuthTag(0xABCD0020));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_08.AddAttribute_MatterFabricId(0xFAB000000000001D));
    EXPECT_TRUE(CHIP_NO_ERROR == subjectDN_Node02_08.AddAttribute_MatterCASEAuthTag(0xABCE0100));
    ...
    for (auto & testCase : sTestCases)
    {
        ...
        EXPECT_TRUE(expectedSubjectLen == subject.size());
        EXPECT_TRUE(memcmp(subject.data(), expectedSubjectBuf, expectedSubjectLen) == 0);
    }
}

TEST_F(TestCryptoPAL, TestIssuer_x509Extraction)
{
    using namespace TestCerts;
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == issuerDN_Root01.AddAttribute_MatterRCACId(0xCACACACA00000001));
    ChipDN issuerDN_ICA02;
    EXPECT_TRUE(CHIP_NO_ERROR == issuerDN_ICA02.AddAttribute_MatterRCACId(0xCACACACA00000002));
    EXPECT_TRUE(CHIP_NO_ERROR == issuerDN_ICA02.AddAttribute_MatterFabricId(0xFAB000000000001D));
    ChipDN issuerDN_Node02_02;
    EXPECT_TRUE(CHIP_NO_ERROR == issuerDN_Node02_02.AddAttribute_MatterICACId(0xCACACACA00000004));
    EXPECT_TRUE(CHIP_NO_ERROR == issuerDN_Node02_02.AddAttribute_MatterFabricId(0xFAB000000000001D));
    ...
    for (auto & testCase : sTestCases)
    {
        ...
        EXPECT_TRUE(expectedIssuerLen == issuer.size());
        EXPECT_TRUE(memcmp(issuer.data(), expectedIssuerBuf, expectedIssuerLen) == 0);
    }
}

TEST_F(TestCryptoPAL, TestVIDPID_StringExtraction)
{
    HeapChecker heapChecker;
    ...
    for (const auto & testCase : kTestCases)
    {
        ...
        EXPECT_TRUE(result == testCase.expectedResult);
        ...
        if (result == CHIP_NO_ERROR)
        {
            ...
            EXPECT_TRUE(vidpidToCheck.mVendorId.HasValue() == testCase.expectedVidPresent);
            EXPECT_TRUE(vidpidToCheck.mProductId.HasValue() == testCase.expectedPidPresent);

            if (testCase.expectedVidPresent)
            {
                EXPECT_TRUE(vidpidToCheck.mVendorId.Value() == testCase.expectedVid);
            }

            if (testCase.expectedPidPresent)
            {
                EXPECT_TRUE(vidpidToCheck.mProductId.Value() == testCase.expectedPid);
            }
        }
        ++caseIdx;
    }
...
TEST_F(TestCryptoPAL, TestVIDPID_x509Extraction)
{
    using namespace TestCerts;
    ...
    EXPECT_TRUE(GetTestCert(TestCert::kNode01_01, TestCertLoadFlags::kDERForm, kOpCertNoVID) == CHIP_NO_ERROR);
    ...
    for (const auto & testCase : kTestCases)
    {
        ...
        EXPECT_TRUE(result == testCase.expectedResult);
        EXPECT_TRUE(vidpid.mVendorId.HasValue() == testCase.expectedVidPresent);
        EXPECT_TRUE(vidpid.mProductId.HasValue() == testCase.expectedPidPresent);

        // If present, make sure the VID matches expectation.
        if (testCase.expectedVidPresent)
        {
            EXPECT_TRUE(vidpid.mVendorId.Value() == testCase.expectedVid);
        }

        // If present, make sure the VID matches expectation.
        if (testCase.expectedPidPresent)
        {
            EXPECT_TRUE(vidpid.mProductId.Value() == testCase.expectedPid);
        }
    }
}

TEST_F(TestCryptoPAL, TestX509_ReplaceCertIfResignedCertFound)
{
    using namespace TestCerts;
    ...
    for (const auto & testCase : kTestCases)
    {
        ...
        EXPECT_TRUE(result == CHIP_NO_ERROR);
    ...
    // Error case: invalid input argument for referenceCertificate
    {
        ...
        EXPECT_TRUE(result == CHIP_ERROR_INVALID_ARGUMENT);
    }
...
TEST_F(TestCryptoPAL, TestGroup_OperationalKeyDerivation)
{
    ...
    // Invalid Epoch Key
    EXPECT_TRUE(CHIP_ERROR_INVALID_ARGUMENT == DeriveGroupOperationalKey(ByteSpan(), compressed_fabric_id, operational_key));

    // Epoch Key 1
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupOperationalKey(epoch_key, compressed_fabric_id, operational_key));
    EXPECT_TRUE(0 == memcmp(operational_key.data(), kGroupOperationalKey1, sizeof(kGroupOperationalKey1)));

    // Epoch Key 2
    epoch_key = ByteSpan(kEpochKeyBuffer2, sizeof(kEpochKeyBuffer2));
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupOperationalKey(epoch_key, compressed_fabric_id, operational_key));
    EXPECT_TRUE(0 == memcmp(operational_key.data(), kGroupOperationalKey2, sizeof(kGroupOperationalKey2)));
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupOperationalKey(epoch_key, compressed_fabric_id, operational_key));
    EXPECT_TRUE(0 == memcmp(operational_key.data(), kGroupOperationalKey3, sizeof(kGroupOperationalKey3)));
}

TEST_F(TestCryptoPAL, TestGroup_SessionIdDerivation)
{
    ...
    // Bad Key
    EXPECT_TRUE(CHIP_ERROR_INVALID_ARGUMENT == DeriveGroupSessionId(ByteSpan(), session_id));

    // Session ID 1
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupSessionId(operational_key1, session_id));
    EXPECT_TRUE(kGroupSessionId1 == session_id);

    // Session ID 2
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupSessionId(operational_key2, session_id));
    EXPECT_TRUE(kGroupSessionId2 == session_id);
}

TEST_F(TestCryptoPAL, TestGroup_PrivacyKeyDerivation)
{
    uint8_t key_buffer[Crypto::CHIP_CRYPTO_SYMMETRIC_KEY_LENGTH_BYTES] = { 0 };
    ...
    // Invalid Epoch Key
    EXPECT_TRUE(CHIP_ERROR_INVALID_ARGUMENT == DeriveGroupPrivacyKey(ByteSpan(), privacy_key));

    // Epoch Key 1
    encryption_key = ByteSpan(kGroupOperationalKey1, sizeof(kGroupOperationalKey1));
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupPrivacyKey(encryption_key, privacy_key));
    EXPECT_TRUE(0 == memcmp(privacy_key.data(), kGroupPrivacyKey1, sizeof(kGroupPrivacyKey1)));

    // Epoch Key 2
    encryption_key = ByteSpan(kGroupOperationalKey2, sizeof(kGroupOperationalKey2));
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupPrivacyKey(encryption_key, privacy_key));
    EXPECT_TRUE(0 == memcmp(privacy_key.data(), kGroupPrivacyKey2, sizeof(kGroupPrivacyKey2)));

    // Epoch Key 3 (example from spec)
    encryption_key = ByteSpan(kGroupOperationalKey3, sizeof(kGroupOperationalKey3));
    EXPECT_TRUE(CHIP_NO_ERROR == DeriveGroupPrivacyKey(encryption_key, privacy_key));
    EXPECT_TRUE(0 == memcmp(privacy_key.data(), kGroupPrivacyKey3, sizeof(kGroupPrivacyKey3)));
}
