...
class TestSessionKeystore : public ::testing::Test
{
public:
...
TEST_F(TestSessionKeystore, TestBasicImport)
{
    ...
    for (const ccm_128_test_vector * testPtr : ccm_128_test_vectors)
    {
        ...
        EXPECT_TRUE(AES_CCM_encrypt(test.pt, test.pt_len, test.aad, test.aad_len, keyHandle, test.nonce, test.nonce_len,
                                    ciphertext.Get(), tag.Get(), test.tag_len) == test.result);
        EXPECT_TRUE(memcmp(ciphertext.Get(), test.ct, test.ct_len) == 0);
        EXPECT_TRUE(memcmp(tag.Get(), test.tag, test.tag_len) == 0);
...
TEST_F(TestSessionKeystore, TestDeriveKey)
{
    ...
    for (const DeriveKeyTestVector & test : deriveKeyTestVectors)
    {
        ...
        Aes128KeyHandle keyHandle;
        ASSERT_EQ(CHIP_NO_ERROR, keystore.DeriveKey(secret, ToSpan(test.salt), ToSpan(test.info), keyHandle));

        uint8_t ciphertext[sizeof(test.ciphertext)];
        ASSERT_EQ(CHIP_NO_ERROR,
                  AES_CTR_crypt(test.plaintext, sizeof(test.plaintext), keyHandle, test.nonce, sizeof(test.nonce), ciphertext));
        EXPECT_TRUE(memcmp(ciphertext, test.ciphertext, sizeof(test.ciphertext)) == 0);
...
TEST_F(TestSessionKeystore, TestDeriveSessionKeys)
{
    ...
    for (const DeriveSessionKeysTestVector & test : deriveSessionKeysTestVectors)
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR,
                  keystore.DeriveSessionKeys(ToSpan(test.secret), ToSpan(test.salt), ToSpan(test.info), i2r, r2i, challenge));

        uint8_t ciphertext[sizeof(test.i2rCiphertext)];

        // Test I2R key
        ASSERT_EQ(CHIP_NO_ERROR,
                  AES_CTR_crypt(test.plaintext, sizeof(test.plaintext), i2r, test.nonce, sizeof(test.nonce), ciphertext));
        EXPECT_TRUE(memcmp(ciphertext, test.i2rCiphertext, sizeof(test.i2rCiphertext)) == 0);

        // Test R2I key
        ASSERT_EQ(CHIP_NO_ERROR,
                  AES_CTR_crypt(test.plaintext, sizeof(test.plaintext), r2i, test.nonce, sizeof(test.nonce), ciphertext));
        EXPECT_TRUE(memcmp(ciphertext, test.r2iCiphertext, sizeof(test.r2iCiphertext)) == 0);

        // Check attestation challenge
        EXPECT_TRUE(memcmp(challenge.Bytes(), test.attestationChallenge, sizeof(test.attestationChallenge)) == 0);
