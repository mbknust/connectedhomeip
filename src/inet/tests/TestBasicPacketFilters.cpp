...
TEST_F(TestBasicPacketFilters, TestBasicPacketFilter)
{
    ...
    auto predicate = [](void * context, const void * endpoint, const chip::Inet::IPPacketInfo & pktInfo,
        ...
        EXPECT_TRUE(context == &gFilter);
        EXPECT_TRUE(endpoint == expectedEndpoint);
...

    {
        ...
        {
            EXPECT_TRUE(kAllowPacket == fakeUdpEndpoint.ProcessEnqueue(fakeSrc, kOtherPort, fakeDest, kOtherPort, kFakePayload));
        }
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 0);

        // Dequeue all packets
        for (int numPkt = 0; numPkt < (kMaxQueuedPacketsLimit + 1); ++numPkt)
        {
            EXPECT_TRUE(kAllowPacket == fakeUdpEndpoint.ProcessDequeue(fakeSrc, kOtherPort, fakeDest, kOtherPort, kFakePayload));
        }
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 0);

        // OnDroped/OnLastMatchDequeued only ever called for matching packets, never for non-matching
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 0);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 0);
    }

    {
        ...
        {
            EXPECT_TRUE(kAllowPacket == fakeUdpEndpoint.ProcessEnqueue(fakeSrc, kOtherPort, fakeMdnsDest, kMdnsPort, kFakePayload));
        }
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 0);
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 0);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 0);
        ...
        {
            EXPECT_TRUE(kDropPacket == fakeUdpEndpoint.ProcessEnqueue(fakeSrc, kOtherPort, fakeMdnsDest, kMdnsPort, kFakePayload));
        }
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 2);
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 2);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 0);

        // Dequeue 2 packets that were enqueued, matching filter
        for (int numPkt = 0; numPkt < 2; ++numPkt)
        {
            EXPECT_TRUE(kAllowPacket == fakeUdpEndpoint.ProcessDequeue(fakeSrc, kOtherPort, fakeMdnsDest, kMdnsPort, kFakePayload));
        }
        ...
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 4);
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 4);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 0);
        ...
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 4);
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 4);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 1);
    }
    ...
    // Validate that setting max packets to zero, with a matching predicate, drops all matching packets, none of the non-matching.
    {
        ...
        EXPECT_TRUE(gFilter.GetNumDroppedPackets() == 3);
        EXPECT_TRUE(gFilter.mNumOnDroppedCalled == 3);
        EXPECT_TRUE(gFilter.mNumOnLastMatchDequeuedCalled == 0);
