...
TEST_F(TestInetEndPoint, TestInetInterface)
{
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR && intName[0] == '\0');
    ...
    for (; intIterator.HasCurrent(); intIterator.Next())
    {
        ...
        if (err == CHIP_NO_ERROR)
        {
            ...
            EXPECT_TRUE(intIterator.GetHardwareAddress(nullptr, intHwAddressSize, sizeof(intHwAddress)) ==
                        CHIP_ERROR_INVALID_ARGUMENT);
            EXPECT_TRUE(intIterator.GetHardwareAddress(intHwAddress, intHwAddressSize, 4) == CHIP_ERROR_BUFFER_TOO_SMALL);
        }
    }

    EXPECT_TRUE(!intIterator.Next());
    EXPECT_TRUE(intIterator.GetInterfaceId() == InterfaceId::Null());
    EXPECT_TRUE(intIterator.GetInterfaceName(intName, sizeof(intName)) == CHIP_ERROR_INCORRECT_STATE);
    EXPECT_TRUE(!intIterator.SupportsMulticast());
    EXPECT_TRUE(!intIterator.HasBroadcastAddress());
    ...
    EXPECT_TRUE(!addrIterator.Next());
    EXPECT_TRUE(addrIterator.GetAddress(addr) == CHIP_ERROR_SENTINEL);
    EXPECT_TRUE(addrIterator.GetInterfaceId() == InterfaceId::Null());
    EXPECT_TRUE(addrIterator.GetInterfaceName(intName, sizeof(intName)) == CHIP_ERROR_INCORRECT_STATE);
    EXPECT_TRUE(!addrIterator.SupportsMulticast());
    EXPECT_TRUE(!addrIterator.HasBroadcastAddress());
}
...
TEST_F(TestInetEndPoint, TestInetEndPointInternal)
{
    ...
    EXPECT_TRUE(err == INET_ERROR_WRONG_ADDRESS_TYPE);
    ...
    EXPECT_TRUE(err == CHIP_ERROR_INCORRECT_STATE);
    EXPECT_TRUE(!testTCPEP1->PendingReceiveLength());
...
TEST_F(TestInetEndPoint, TestInetEndPointLimit)
{
    ...
    for (int i = INET_CONFIG_NUM_UDP_ENDPOINTS; i >= 0; --i)
    {
        ...
        EXPECT_TRUE(err == (i ? CHIP_NO_ERROR : CHIP_ERROR_ENDPOINT_POOL_FULL));
        if (err == CHIP_NO_ERROR)
        {
            ++udpCount;
    ...
    EXPECT_TRUE(err == CHIP_ERROR_NO_MEMORY);
