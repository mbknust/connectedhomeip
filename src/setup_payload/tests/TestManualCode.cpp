...
{
    EXPECT_TRUE(actualError == expectedError);
    EXPECT_TRUE(payload.setUpPINCode == pinCode);
    EXPECT_TRUE(payload.discriminator == discriminator);
    EXPECT_TRUE(payload.vendorID == vendorID);
    EXPECT_TRUE(payload.productID == productID);
}
...
TEST(TestManualCode, TestGenerateAndParser_ManualSetupCodeWithLongDiscriminator)
{
...

    {
        ...
        ManualSetupPayloadGenerator generator(payload);
        std::string result;
        EXPECT_TRUE(generator.payloadDecimalStringRepresentation(result) == CHIP_NO_ERROR);
...
TEST(TestManualCode, TestDecimalRepresentation_AllZeros)
{
    ...
    EXPECT_TRUE(CheckGenerator(payload, expectedResult) == false);
}
...
TEST(TestManualCode, TestPayloadParser_PartialPayload)
{
    ...
    decimalString += Verhoeff10::ComputeCheckChar(decimalString.c_str());
    EXPECT_TRUE(decimalString.length() == 11);
    err = ManualSetupPayloadParser(decimalString).populatePayload(payload);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    ...
    decimalString += ComputeCheckChar(decimalString);
    EXPECT_TRUE(decimalString.length() == 13);
    err = ManualSetupPayloadParser(decimalString).populatePayload(payload);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    ...
    decimalString += Verhoeff10::ComputeCheckChar(decimalString.c_str());
    EXPECT_TRUE(decimalString.length() == 21);
    err = ManualSetupPayloadParser(decimalString).populatePayload(payload);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
...
TEST(TestManualCode, TestCheckDecimalStringValidity)
{
    ...
    EXPECT_TRUE(ManualSetupPayloadParser::CheckDecimalStringValidity(representationWithoutCheckDigit, outReprensation) ==
    ...
    EXPECT_TRUE(ManualSetupPayloadParser::CheckDecimalStringValidity(decimalString, outReprensation) == CHIP_NO_ERROR);
    EXPECT_TRUE(outReprensation == representationWithoutCheckDigit);
...
{
    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("01234567890123456789", true) == CHIP_NO_ERROR);
    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("0123456789", false) == CHIP_NO_ERROR);

    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("01234567891", false) == CHIP_ERROR_INVALID_STRING_LENGTH);
    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("012345678", false) == CHIP_ERROR_INVALID_STRING_LENGTH);
    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("012345678901234567891", true) ==
                CHIP_ERROR_INVALID_STRING_LENGTH);
    EXPECT_TRUE(ManualSetupPayloadParser::CheckCodeLengthValidity("0123456789012345678", true) == CHIP_ERROR_INVALID_STRING_LENGTH);
}

TEST(TestManualCode, TestDecimalStringToNumber)
{
    uint32_t number;
    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("12345", number) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 12345);

    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("01234567890", number) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 1234567890);

    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("00000001", number) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 1);

    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("0", number) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 0);

    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("012345.123456789", number) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(ManualSetupPayloadParser::ToNumber("/", number) == CHIP_ERROR_INVALID_INTEGER_VALUE);
}
...
TEST(TestManualCode, TestReadCharsFromDecimalString)
{
    uint32_t number;
    size_t index = 3;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("12345", index, number, 2) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 45);

    index = 2;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("6256276377282", index, number, 7) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 5627637);

    index = 0;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("10", index, number, 2) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 10);

    index = 0;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("01", index, number, 2) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 1);

    index = 1;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("11", index, number, 1) == CHIP_NO_ERROR);
    EXPECT_TRUE(number == 1);

    index = 2;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("100001", index, number, 3) == CHIP_NO_ERROR);

    index = 1;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("12345", index, number, 5) ==
                CHIP_ERROR_INVALID_STRING_LENGTH);
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("12", index, number, 5) == CHIP_ERROR_INVALID_STRING_LENGTH);

    index = 200;
    EXPECT_TRUE(ManualSetupPayloadParser::ReadDigitsFromDecimalString("6256276377282", index, number, 1) ==
                CHIP_ERROR_INVALID_STRING_LENGTH);
}
...
TEST(TestManualCode, TestShortCodeCharLengths)
{
    ...
    EXPECT_TRUE(manualSetupShortCodeCharLength == kManualSetupShortCodeCharLength);

    size_t manualSetupVendorIdCharLength = static_cast<size_t>(ceil(log10(pow(2, kVendorIDFieldLengthInBits))));
    EXPECT_TRUE(manualSetupVendorIdCharLength == kManualSetupVendorIdCharLength);

    size_t manualSetupProductIdCharLength = static_cast<size_t>(ceil(log10(pow(2, kProductIDFieldLengthInBits))));
    EXPECT_TRUE(manualSetupProductIdCharLength == kManualSetupProductIdCharLength);
    ...
    EXPECT_TRUE(manualSetupLongCodeCharLength == kManualSetupLongCodeCharLength);
}
