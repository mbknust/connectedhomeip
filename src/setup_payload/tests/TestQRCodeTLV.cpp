...
TEST_F(TestQRCodeTLV, TestOptionalDataAddRemove)
{
    ...
    err = payload.removeOptionalVendorData(kOptionalDefaultStringTag);
    EXPECT_TRUE(err == CHIP_ERROR_KEY_NOT_FOUND);
...
TEST_F(TestQRCodeTLV, TestSerialNumberAddRemove)
{
    ...
    string sn;
    EXPECT_TRUE(inPayload.getSerialNumber(sn) == CHIP_ERROR_KEY_NOT_FOUND);
    EXPECT_TRUE(inPayload.removeSerialNumber() == CHIP_ERROR_KEY_NOT_FOUND);

    EXPECT_TRUE(inPayload.addSerialNumber(kSerialNumberDefaultStringValue) == CHIP_NO_ERROR);
    EXPECT_TRUE(inPayload.getSerialNumber(sn) == CHIP_NO_ERROR);
    EXPECT_TRUE(sn == kSerialNumberDefaultStringValue);

    EXPECT_TRUE(inPayload.addSerialNumber(kSerialNumberDefaultUInt32Value) == CHIP_NO_ERROR);
    EXPECT_TRUE(inPayload.getSerialNumber(sn) == CHIP_NO_ERROR);
    EXPECT_TRUE(sn == to_string(kSerialNumberDefaultUInt32Value));

    EXPECT_TRUE(inPayload.removeSerialNumber() == CHIP_NO_ERROR);
    EXPECT_TRUE(inPayload.getSerialNumber(sn) == CHIP_ERROR_KEY_NOT_FOUND);
    EXPECT_TRUE(inPayload.removeSerialNumber() == CHIP_ERROR_KEY_NOT_FOUND);
}
