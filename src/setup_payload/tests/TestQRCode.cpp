...
TEST(TestQRCode, TestPayloadBase38Rep)
{
    ...
    EXPECT_TRUE(didSucceed == true);

    EXPECT_TRUE(result == kDefaultPayloadQRCode);
}
...
TEST(TestQRCode, TestBase38)
{
    ...
    EXPECT_TRUE(encodedLen == strlen("A0"));
    EXPECT_TRUE(strcmp(encodedBuf, "A0") == 0);

    // passing empty parameters
    MutableCharSpan emptySpan;
    encodedSpan = MutableCharSpan(encodedBuf);
    EXPECT_TRUE(base38Encode(inputSpan, emptySpan) == CHIP_ERROR_BUFFER_TOO_SMALL);
    base38Encode(MutableByteSpan(), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "") == 0);
    EXPECT_TRUE(base38Encode(MutableByteSpan(), emptySpan) == CHIP_ERROR_BUFFER_TOO_SMALL);

    // test single odd byte corner conditions
    encodedSpan = MutableCharSpan(encodedBuf);
    input[2]    = 0;
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "OT100") == 0);
    input[2]    = 40;
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "Y6V91") == 0);
    input[2]    = 41;
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "KL0B1") == 0);
    input[2]    = 255;
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "Q-M08") == 0);
    ...
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan.SubSpan(0, 1), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "Z0") == 0);
    // for 2 bytes we need always 4 characters
    input[0]    = 255;
    input[1]    = 0;
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan.SubSpan(0, 2), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "R600") == 0);
    ...
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "81000") == 0);
    ...
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan.SubSpan(0, 1), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "R6") == 0);
    // for 2 bytes we need 4 characters
    input[0]    = 255;
    input[1]    = 255;
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan.SubSpan(0, 2), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "NE71") == 0);
    ...
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(inputSpan, encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "PLS18") == 0);

    // fun with strings
    encodedSpan = MutableCharSpan(encodedBuf);
    base38Encode(ByteSpan((uint8_t *) "Hello World!", sizeof("Hello World!") - 1), encodedSpan);
    EXPECT_TRUE(strcmp(encodedBuf, "KKHF3W2S013OPM3EJX11") == 0);

    vector<uint8_t> decoded = vector<uint8_t>();
    EXPECT_TRUE(base38Decode("KKHF3W2S013OPM3EJX11", decoded) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(hello_world == "Hello World!");

    // short input
    EXPECT_TRUE(base38Decode("A0", decoded) == CHIP_NO_ERROR);
    EXPECT_TRUE(decoded.size() == 1);
    EXPECT_TRUE(decoded[0] == 10);

    // empty == empty
    EXPECT_TRUE(base38Decode("", decoded) == CHIP_NO_ERROR);
    EXPECT_TRUE(decoded.empty());

    // test invalid characters
    EXPECT_TRUE(base38Decode("0\001", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode("\0010", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode("[0", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode("0[", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode(" 0", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode("!0", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    EXPECT_TRUE(base38Decode("\"0", decoded) == CHIP_ERROR_INVALID_INTEGER_VALUE);
    ...
    EXPECT_TRUE(base38Decode("R6", decoded) == CHIP_NO_ERROR); // this is 0xFF
    EXPECT_TRUE(decoded == std::vector<uint8_t>({ 255 }));
    EXPECT_TRUE(base38Decode("S6", decoded) == CHIP_ERROR_INVALID_ARGUMENT); // trying to encode 0xFF + 1 in 2 chars
    EXPECT_TRUE(base38Decode("S600", decoded) == CHIP_NO_ERROR);             // this is 0xFF + 1, needs 4 chars
    EXPECT_TRUE(decoded == std::vector<uint8_t>({ 0, 1 }));
    EXPECT_TRUE(base38Decode("NE71", decoded) == CHIP_NO_ERROR); // this is 0xFFFF
    EXPECT_TRUE(decoded == std::vector<uint8_t>({ 255, 255 }));
    EXPECT_TRUE(base38Decode("OE71", decoded) == CHIP_ERROR_INVALID_ARGUMENT); // trying to encode 0xFFFF + 1 in 4 chars
    EXPECT_TRUE(base38Decode("OE710", decoded) == CHIP_NO_ERROR);              // this is 0xFFFF + 1, needs 5 chars
    EXPECT_TRUE(decoded == std::vector<uint8_t>({ 0, 0, 1 }));
    EXPECT_TRUE(base38Decode("PLS18", decoded) == CHIP_NO_ERROR); // this is 0xFFFFFF
    EXPECT_TRUE(decoded == std::vector<uint8_t>({ 255, 255, 255 }));
    EXPECT_TRUE(base38Decode("QLS18", decoded) == CHIP_ERROR_INVALID_ARGUMENT); // trying to encode 0xFFFFFF + 1
}

TEST(TestQRCode, TestBitsetLen)
{
    EXPECT_TRUE(kTotalPayloadDataSizeInBits % 8 == 0);
}

TEST(TestQRCode, TestSetupPayloadVerify)
{
    SetupPayload payload = GetDefaultPayload();
    EXPECT_TRUE(payload.isValidQRCodePayload() == true);
    ...
    EXPECT_TRUE(test_payload.isValidQRCodePayload() == false);
...
TEST(TestQRCode, TestInvalidQRCodePayload_WrongCharacterSet)
{
    ...
    EXPECT_TRUE(didFail == true);
    EXPECT_TRUE(payload.isValidQRCodePayload() == false);
}
...
TEST(TestQRCode, TestPayloadInEquality)
{
    ...
    EXPECT_TRUE(!(payload == unequalPayload));
}
...
TEST(TestQRCode, TestQRCodeToPayloadGeneration)
{
    ...
    EXPECT_TRUE(didSucceed == true);
    EXPECT_TRUE(resultingPayload.isValidQRCodePayload() == true);
...
{
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("MT:ABC")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("MT:")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("H:")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("ASMT:")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("Z%MT:ABC%")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("Z%MT:ABC")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%Z%MT:ABC")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%Z%MT:ABC%")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%Z%MT:ABC%DDD")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("MT:ABC%DDD")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("MT:ABC%")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%MT:")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%MT:%")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("A%")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("MT:%")) == string(""));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("%MT:ABC")) == string("ABC"));
    EXPECT_TRUE(QRCodeSetupPayloadParser::ExtractPayload(string("ABC")) == string(""));
}
