...
TEST_F(TestJsonToTlv, Test32BitConvert)
{
    ...
    // convert a simple single value
    {
        ...
        EXPECT_TRUE(gWriter1.Finalize() == CHIP_NO_ERROR);

        reader.Init(gBuf1, gWriter1.GetLengthWritten());
        reader.ImplicitProfileId = kImplicitProfileId;

        EXPECT_TRUE(reader.Next(TLV::AnonymousTag()) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_Structure);
        EXPECT_TRUE(reader.EnterContainer(tlvType) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.Next(TLV::ContextTag(1)) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_SignedInteger);
        EXPECT_TRUE(reader.Get(value) == CHIP_NO_ERROR);
        EXPECT_TRUE(value == 321);
        EXPECT_TRUE(reader.Next() == CHIP_END_OF_TLV);
        EXPECT_TRUE(reader.ExitContainer(tlvType) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.Next() == CHIP_END_OF_TLV);
    }
    ...
    // convert a single value that is larger than 8 bit
    {
        ...
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_Structure);
        EXPECT_TRUE(reader.EnterContainer(tlvType) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.Next(TLV::ProfileTag(kImplicitProfileId, 1234)) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_SignedInteger);
        EXPECT_TRUE(reader.Get(value) == CHIP_NO_ERROR);
        EXPECT_TRUE(value == 321);
        EXPECT_TRUE(reader.Next() == CHIP_END_OF_TLV);
    ...
    // Convert to a full 32-bit value, unsigned
    {
        ...
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_Structure);
        EXPECT_TRUE(reader.EnterContainer(tlvType) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.Next(TLV::ProfileTag(kImplicitProfileId, 0xFEDCBA98u)) == CHIP_NO_ERROR);
        EXPECT_TRUE(reader.GetType() == TLV::kTLVType_SignedInteger);
        EXPECT_TRUE(reader.Get(value) == CHIP_NO_ERROR);
        EXPECT_TRUE(value == 321);
        EXPECT_TRUE(reader.Next() == CHIP_END_OF_TLV);
