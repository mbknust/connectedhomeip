...
TEST(TestBitMask, TestBitMaskOperations)
{
    ...
    EXPECT_TRUE(mask.Raw() == 0);

    mask.SetField(TestEnum::kBits_1_2, 2);
    EXPECT_TRUE(mask.Raw() == 0x0004);

    mask.SetRaw(0);
    mask.SetField(TestEnum::kBits_4_7, 0x0B);
    EXPECT_TRUE(mask.Raw() == 0x00B0);
    ...
    for (uint16_t i = 0; i < 0x10; i++)
    {
        ...
        EXPECT_TRUE(mask.Raw() == (i << 12));
    }

    mask.SetField(TestEnum::kBits_High8, 0x23);
    EXPECT_TRUE(mask.Raw() == 0x2300);

    mask.SetField(TestEnum::kBits_High4, 0xA);
    EXPECT_TRUE(mask.Raw() == 0xA300);
}

TEST(BitMask, TestBitFieldLogic)
{
    BitMask<TestEnum> mask;
...
TEST(BitMask, TestBitMaskInvalid)
{
    BitMask<TestEnum> mask;
