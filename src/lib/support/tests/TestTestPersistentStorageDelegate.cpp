...
TEST(TestPersistentStorageDelegate, TestBasicApi)
{
    ...
    EXPECT_TRUE(size == actualSizeOfBuf);

    EXPECT_TRUE(storage.GetNumKeys() == 0);
    ...
    err  = storage.SyncGetKeyValue("roboto", &buf[0], size);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(size == strlen(kStringValue1));
    EXPECT_TRUE(0 == memcmp(&buf[0], kStringValue1, strlen(kStringValue1)));
    ...
    EXPECT_TRUE(storage.GetNumKeys() == 2);
    auto keys = storage.GetKeys();
    std::array<std::string, 2> kExpectedKeys{ "key2", "key3" };
    EXPECT_TRUE(SetMatches(keys, kExpectedKeys) == true);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(size == strlen(kStringValue2));
    EXPECT_TRUE(0 == memcmp(&buf[0], kStringValue2, strlen(kStringValue2)));
    // Make sure that there was no buffer overflow during SyncGetKeyValue
    EXPECT_TRUE(0 == memcmp(&buf[size], &all_zeroes[0], sizeof(buf) - size));

    memset(&buf[0], 0, sizeof(buf));
    size = actualSizeOfBuf;
    err  = storage.SyncGetKeyValue("key3", &buf[0], size);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(size == strlen(kStringValue3));
    EXPECT_TRUE(0 == memcmp(&buf[0], kStringValue3, strlen(kStringValue3)));
    // Make sure that there was no buffer overflow during SyncGetKeyValue
    EXPECT_TRUE(0 == memcmp(&buf[size], &all_zeroes[0], sizeof(buf) - size));
    ...
    EXPECT_TRUE(size == sizeBeforeGetKeyValueCall);
    EXPECT_TRUE(0 == memcmp(&buf[0], kStringValue2, size));
    // Make sure that there was no buffer overflow during SyncGetKeyValue
    EXPECT_TRUE(0 == memcmp(&buf[size], &all_zeroes[0], sizeof(buf) - size));
    ...
    // nullptr is provided.
    EXPECT_TRUE(0 == memcmp(&buf[0], &all_zeroes[0], sizeof(buf)));
    ...
    EXPECT_TRUE(err == CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
    EXPECT_TRUE(actualSizeOfBuf == size);
    EXPECT_TRUE(0 == memcmp(&buf[0], &all_zeroes[0], sizeof(buf)));
    ...
    EXPECT_TRUE(err == CHIP_ERROR_INVALID_ARGUMENT);
    EXPECT_TRUE(actualSizeOfBuf == size);
    EXPECT_TRUE(0 == memcmp(&buf[0], &all_zeroes[0], size));
    ...
    EXPECT_TRUE(err == CHIP_ERROR_INVALID_ARGUMENT);
    EXPECT_TRUE(!storage.SyncDoesKeyExist("key4"));
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(size == strlen(kBase64SymbolValues));
    EXPECT_TRUE(0 == memcmp(&buf[0], kBase64SymbolValues, strlen(kBase64SymbolValues)));
    // Make sure that there was no buffer overflow during SyncGetKeyValue
    EXPECT_TRUE(0 == memcmp(&buf[size], &all_zeroes[0], sizeof(buf) - size));

    err = storage.SyncDeleteKeyValue(kBase64SymbolsKey);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(!storage.SyncDoesKeyExist(kBase64SymbolsKey));
    ...
    EXPECT_TRUE(strlen(longKeyString) == PersistentStorageDelegate::kKeyLengthMax);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(size == static_cast<uint16_t>(sizeof(largeBuffer)));
    EXPECT_TRUE(0 == memcmp(&largeBuffer, largeBufferForCheck, sizeof(largeBuffer)));
...
TEST(TestPersistentStorageDelegate, TestClearStorage)
{
    TestPersistentStorageDelegate storage;
    ...
    err  = storage.SyncGetKeyValue("roboto", &buf[0], size);
    EXPECT_TRUE(err == CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND);
    EXPECT_TRUE(size == sizeof(buf));

    // Add basic key, read it back
    const char * kStringValue1 = "abcd";
    err                        = storage.SyncSetKeyValue("roboto", kStringValue1, static_cast<uint16_t>(strlen(kStringValue1)));
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(storage.GetNumKeys() == 1);
