...
TEST(BufferWriter, TestSpanVersusRegular)
{
    ...
    EXPECT_TRUE(regular_writer.Available() == sizeof(buf_regular));
    EXPECT_TRUE(span_writer.Available() == sizeof(buf_span));

    EXPECT_TRUE(0 == memcmp(buf_regular, all_zeroes, sizeof(all_zeroes)));
    EXPECT_TRUE(0 == memcmp(buf_span, all_zeroes, sizeof(all_zeroes)));

    EXPECT_TRUE(regular_writer.Put(1).Put(2).Put(3).Put(4).Fit());
    EXPECT_TRUE(span_writer.Put(1).Put(2).Put(3).Put(4).Fit());

    EXPECT_TRUE(0 == memcmp(buf_regular, final_expected, sizeof(final_expected)));
    EXPECT_TRUE(0 == memcmp(buf_span, final_expected, sizeof(final_expected)));
}

TEST(BufferWriter, TestStringWrite)
{
    {
...
TEST(BufferWriter, TestBufferWrite)
{
    {
        BWTest<BufferWriter> bb(2);
        bb.Put("hithere", 2);
...
TEST(BufferWriter, TestPutLittleEndian)
{
    {
        BWTest<LittleEndian::BufferWriter> bb(2);
        bb.Put16('h' + 'i' * 256);
...
TEST(BufferWriter, TestPutBigEndian)
{
    {
        BWTest<BigEndian::BufferWriter> bb(2);
        bb.Put16('i' + 'h' * 256);
