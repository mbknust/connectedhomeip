...
TEST_F(TestZclString, WhenBufferIsZero)
{
    ...
    EXPECT_TRUE(err == CHIP_ERROR_INBOUND_MESSAGE_TOO_BIG);
    EXPECT_TRUE(zclString.data()[0] == 0);
}

TEST_F(TestZclString, LessThanMaximumSize_Length_64)
{
    ...
    EXPECT_TRUE(zclString.data()[0] == 64);
    EXPECT_TRUE(allCharactersSame(zclString.data()) == true);
}

TEST_F(TestZclString, EqualsMaximumSize)
{
    uint8_t bufferMemory[256];
    MutableByteSpan zclString(bufferMemory);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(zclString.data()[0] == 254);
    EXPECT_TRUE(allCharactersSame(zclString.data()) == true);
}
...
TEST_F(TestZclString, BiggerThanMaximumSize_Length_257)
{
