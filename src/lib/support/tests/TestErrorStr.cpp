...
const char * CheckAndSkipSource(const char * s, const char * file, unsigned int line)
{
    ...
    EXPECT_TRUE(strncmp(s, file, fileLength) == 0);
    EXPECT_TRUE(s[fileLength] == ':');
    char * end;
    EXPECT_TRUE(strtoul(&s[fileLength + 1], &end, 10) == line);
    EXPECT_TRUE(strncmp(end, ": ", 2) == 0);
    return end + 2;
}
...
TEST(TestErrorStr, CheckRegisterDeregisterErrorFormatter)
{
    ...
    EXPECT_TRUE(falseFormatCalled == 1);
    // reset
    falseFormatCalled = 0;
    ...
    ErrorStr(CHIP_ERROR_INTERNAL);
    EXPECT_TRUE(falseFormatCalled == 1);
    EXPECT_TRUE(falseFormat2Called == 1);
    // reset
    falseFormatCalled  = 0;
    ...
    ErrorStr(CHIP_ERROR_INTERNAL);
    EXPECT_TRUE(trueFormatCalled == 1);
    EXPECT_TRUE(falseFormatCalled == 0);
    EXPECT_TRUE(falseFormat2Called == 0);
    // reset
    trueFormatCalled = 0;
    ...
    ErrorStr(CHIP_ERROR_INTERNAL);
    EXPECT_TRUE(trueFormatCalled == 0);
    EXPECT_TRUE(falseFormatCalled == 1);
    EXPECT_TRUE(falseFormat2Called == 1);
...
{
    EXPECT_TRUE(strcmp(CHECK_AND_SKIP_SOURCE(ErrorStr(CHIP_NO_ERROR)), CHIP_NO_ERROR_STRING) == 0);
}
...
TEST(TestErrorStr, CheckFormatErr)
{
    ...
    EXPECT_TRUE(strcmp(buf, "subsys Error 0x00000001: desc") == 0);

    // skip desc
    FormatError(buf, kBufSize, subsys, CHIP_CORE_ERROR(1), nullptr);
    EXPECT_TRUE(strcmp(buf, "subsys Error 0x00000001") == 0);

    // skip subsys
    FormatError(buf, kBufSize, nullptr, CHIP_CORE_ERROR(1), desc);
    EXPECT_TRUE(strcmp(buf, "Error 0x00000001: desc") == 0);

    // skip both
    FormatError(buf, kBufSize, nullptr, CHIP_CORE_ERROR(1), nullptr);
    EXPECT_TRUE(strcmp(buf, "Error 0x00000001") == 0);
#endif
}
