...
TEST_F(TestBytesCircularBuffer, TestPushInvalid)
{
    ...
    EXPECT_TRUE(buffer.Push(ByteSpan(s, static_cast<size_t>(-1))) == CHIP_ERROR_INVALID_ARGUMENT);
    EXPECT_TRUE(buffer.IsEmpty());

    EXPECT_TRUE(buffer.Push(ByteSpan(s, 8)) == CHIP_ERROR_INVALID_ARGUMENT);
    EXPECT_TRUE(buffer.IsEmpty());
    EXPECT_TRUE(buffer.Push(ByteSpan(s, 9)) == CHIP_ERROR_INVALID_ARGUMENT);
    EXPECT_TRUE(buffer.IsEmpty());
    EXPECT_TRUE(buffer.Push(ByteSpan(s, 7)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
}
...
void RandomPushPop(BytesCircularBuffer & buffer, std::list<std::vector<uint8_t>> & alreadyInBuffer)
{
    ...
    auto verify = [&alreadyInBuffer, &buffer] {
        ...
        else
        {
            ...
            EXPECT_TRUE(buffer.GetFrontSize() == length);
            std::vector<uint8_t> str(length);
            MutableByteSpan readSpan(str.data(), length);
            EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
            EXPECT_TRUE(readSpan.size() == length);
            EXPECT_TRUE(alreadyInBuffer.front() == str);
        }
    };
    ...
    for (int i = 0; i < 200; ++i)
    {
        ...
        {
            // pop
            EXPECT_TRUE(buffer.Pop() == CHIP_NO_ERROR);
        ...
        else
        {
            ...
            EXPECT_TRUE(buffer.Push(ByteSpan(str.data(), str.size())) == CHIP_NO_ERROR);
...
TEST_F(TestBytesCircularBuffer, TestPushToJustFull)
{
    ...
    // Used = 6, Free = 3
    EXPECT_TRUE(buffer.Push(ByteSpan(s1, 4)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 4);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s1, readSpan.data(), 4) == 0);

    // Used = 9, Free = 0
    EXPECT_TRUE(buffer.Push(ByteSpan(s2, 1)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 4);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
...
TEST_F(TestBytesCircularBuffer, TestPushToJustFullOverOne)
{
    ...
    // Used = 4, Free = 5
    EXPECT_TRUE(buffer.Push(ByteSpan(s2, 2)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 2);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s2, readSpan.data(), 2) == 0);
...
TEST_F(TestBytesCircularBuffer, TestPushWrap)
{
    ...
    EXPECT_TRUE(buffer.Push(ByteSpan(s2, 4)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 4);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s2, readSpan.data(), 4) == 0);

    // Used = 9, Free = 0
    EXPECT_TRUE(buffer.Push(ByteSpan(s1, 1)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 4);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(buffer.Pop() == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 1);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s1, readSpan.data(), 1) == 0);
...
TEST_F(TestBytesCircularBuffer, TestPushWhenFull)
{
    ...
    // Used = 9, Free = 0
    EXPECT_TRUE(buffer.Push(ByteSpan(s1, 7)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 7);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s1, readSpan.data(), 7) == 0);

    // Used = 5, Free = 4
    EXPECT_TRUE(buffer.Push(ByteSpan(s2, 3)) == CHIP_NO_ERROR);
    EXPECT_TRUE(!buffer.IsEmpty());
    EXPECT_TRUE(buffer.GetFrontSize() == 3);
    readSpan = MutableByteSpan(o);
    EXPECT_TRUE(buffer.ReadFront(readSpan) == CHIP_NO_ERROR);
    EXPECT_TRUE(memcmp(s2, readSpan.data(), 3) == 0);
