...
public:
                           ...
                           DuplicateMessage isDuplicate, System::PacketBufferHandle && msgBuf) override
    {
        ...
        EXPECT_TRUE(header.GetSessionId() == testEntry.sessionId);

        size_t dataLength   = msgBuf->DataLength();
        size_t expectLength = testEntry.payloadLength;

        EXPECT_TRUE(dataLength == expectLength);
        EXPECT_TRUE(memcmp(msgBuf->Start(), testEntry.payload, dataLength) == 0);
...
TestContext TestSessionManagerDispatch::ctx;
...
TEST_F(TestSessionManagerDispatch, TestSessionManagerDispatch)
{
    ...
    TestSessionManagerInit(ctx, sessionManager);
    ...
    for (unsigned i = 0; i < theMessageTestVectorLength; i++)
    {
        ...
        EXPECT_TRUE(callback.NumMessagesReceived() == testEntry.expectedMessageCount);
