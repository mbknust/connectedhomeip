...
TEST(TestPeerMessageCounter, GroupRollOverTest)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = 1; k <= 2 * CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; k++)
        {
            ...
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n) == CHIP_NO_ERROR);

            counter.CommitGroup(n);

            // 1. A counter value of N + k comes in, we detect it as valid and commit it.
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n + k) == CHIP_NO_ERROR);
            counter.CommitGroup(n + k);

            // 2. A counter value of N comes in, we detect it as duplicate.
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n) == CHIP_ERROR_DUPLICATE_MESSAGE_RECEIVED);
            ...
            for (uint32_t i = n - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; i != (n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE);
                 i++)
            {
                EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(i) != CHIP_NO_ERROR);
            }
            ...
            {
                EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) == CHIP_NO_ERROR);
            }
            else
            {
                EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) != CHIP_NO_ERROR);
...
TEST(TestPeerMessageCounter, GroupBackTrackTest)
{
    for (auto n : counterValuesArray)
    {
        ...
        {
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n - (k * k)) == CHIP_NO_ERROR);
            counter.CommitGroup(n - (k * k));
        }
        // 2. Counter value N + 3 comes in
        EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n + 3) == CHIP_NO_ERROR);
        ...
        for (uint32_t k = n + 3 - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; k != n + 3; ++k)
        {
            ...
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(k) == CHIP_NO_ERROR);
...
TEST(TestPeerMessageCounter, GroupBigLeapTest)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = (static_cast<uint32_t>(1 << 31) - 5); k <= (static_cast<uint32_t>(1 << 31) - 1); k++)
        {
            ...
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) == CHIP_NO_ERROR);

            for (auto it : testValues)
            {
                EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(it) != CHIP_NO_ERROR);
            }
...
TEST(TestPeerMessageCounter, GroupOutOfWindow)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = (static_cast<uint32_t>(1 << 31)); k <= (static_cast<uint32_t>(1 << 31) + 2); k++)
        {
            ...
            EXPECT_TRUE(counter.VerifyOrTrustFirstGroup(n + k) == CHIP_ERROR_DUPLICATE_MESSAGE_RECEIVED);
        }
    }
}
...
TEST(TestPeerMessageCounter, UnicastSmallStepTest)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = 1; k <= 2 * CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; k++)
        {
            ...
            EXPECT_TRUE(counter.VerifyEncryptedUnicast(n) == CHIP_ERROR_DUPLICATE_MESSAGE_RECEIVED);

            // A counter value of N + k comes in, we detect it as valid only if it would not
            // overflow, and commit it.
            if (k > UINT32_MAX - n)
            {
                EXPECT_TRUE(counter.VerifyEncryptedUnicast(n + k) == CHIP_ERROR_DUPLICATE_MESSAGE_RECEIVED);
                // The other tests make no sense if we did not commit N+k as the new max counter.
                continue;
            }

            EXPECT_TRUE(counter.VerifyEncryptedUnicast(n + k) == CHIP_NO_ERROR);
            ...
            {
                EXPECT_TRUE(counter.VerifyEncryptedUnicast(i) != CHIP_NO_ERROR);
            }
            ...
            if ((n + k) >= CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE)
            {
                ...
                {
                    EXPECT_TRUE(counter.VerifyEncryptedUnicast(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) == CHIP_NO_ERROR);
                }
                else
                {
                    EXPECT_TRUE(counter.VerifyEncryptedUnicast(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) != CHIP_NO_ERROR);
...
TEST(TestPeerMessageCounter, UnicastLargeStepTest)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = (static_cast<uint32_t>(1 << 31) - 5); k <= (static_cast<uint32_t>(1 << 31) - 1); k++)
        {
            ...
            // 2. A counter value of N comes in, we detect it as duplicate.
            EXPECT_TRUE(counter.VerifyEncryptedUnicast(n) != CHIP_NO_ERROR);
            ...
            {
                EXPECT_TRUE(counter.VerifyEncryptedUnicast(n - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) != CHIP_NO_ERROR);
            }

            for (auto it : testValues)
            {
                EXPECT_TRUE(counter.VerifyEncryptedUnicast(it) != CHIP_NO_ERROR);
            }
...
TEST(TestPeerMessageCounter, UnencryptedRollOverTest)
{
    for (auto n : counterValuesArray)
    {
        ...
        {
            chip::Transport::PeerMessageCounter counter;
            EXPECT_TRUE(counter.VerifyUnencrypted(n) == CHIP_NO_ERROR);

            counter.CommitUnencrypted(n);

            // 1. A counter value of N + k comes in, we detect it as valid and commit it.
            EXPECT_TRUE(counter.VerifyUnencrypted(n + k) == CHIP_NO_ERROR);
            ...
            if (k <= CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE)
            {
                EXPECT_TRUE(counter.VerifyUnencrypted(n) == CHIP_ERROR_DUPLICATE_MESSAGE_RECEIVED);
            }
            else
            {
                EXPECT_TRUE(counter.VerifyUnencrypted(n) == CHIP_NO_ERROR);
            ...
            if (k != CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE)
            {
                EXPECT_TRUE(counter.VerifyUnencrypted(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) == CHIP_NO_ERROR);
            }
            else
            {
                EXPECT_TRUE(counter.VerifyUnencrypted(n + k - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) != CHIP_NO_ERROR);
...
TEST(TestPeerMessageCounter, UnencryptedBackTrackTest)
{
    for (auto n : counterValuesArray)
    {
        ...
        for (uint32_t k = 1; k * k < CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; k++)
        {
            EXPECT_TRUE(counter.VerifyUnencrypted(n - (k * k)) == CHIP_NO_ERROR);
            counter.CommitUnencrypted(n - (k * k));
        }
        // 2. Counter value N + 3 comes in
        EXPECT_TRUE(counter.VerifyUnencrypted(n + 3) == CHIP_NO_ERROR);
        ...
        for (uint32_t k = n + 3 - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE; k != n + 3; ++k)
        {
            ...
            EXPECT_TRUE(counter.VerifyUnencrypted(k) == CHIP_NO_ERROR);
...
TEST(TestPeerMessageCounter, UnencryptedBigLeapTest)
{
    for (auto n : counterValuesArray)
    {
        for (uint32_t k = (static_cast<uint32_t>(1 << 31) - 5); k <= (static_cast<uint32_t>(1 << 31) - 1); k++)
        {
            ...
            // Will be inside the valid window of counter + (2^31 -1)
            EXPECT_TRUE(counter.VerifyUnencrypted(n - CHIP_CONFIG_MESSAGE_COUNTER_WINDOW_SIZE) == CHIP_NO_ERROR);

            for (auto it : testValues)
            {
                EXPECT_TRUE(counter.VerifyUnencrypted(it) == CHIP_NO_ERROR);
            }
