...
TEST_F(TestPeerConnections, TestBasicFunctionality)
{
    ...
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetSecureSessionType() == SecureSession::Type::kCASE);
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetPeerNodeId() == kCasePeer1NodeId);
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetLocalNodeId() == kLocalNodeId);
    EXPECT_TRUE(optionalSession.Value()->GetPeer() == ScopedNodeId(kCasePeer1NodeId, kFabricIndex));
    EXPECT_TRUE(optionalSession.Value()->GetLocalScopedNodeId() == ScopedNodeId(kLocalNodeId, kFabricIndex));
    peerCATs = optionalSession.Value()->AsSecureSession()->GetPeerCATs();
    EXPECT_TRUE(memcmp(&peerCATs, &kPeer1CATs, sizeof(CATValues)) == 0);
    ...
    EXPECT_TRUE(optionalSession.HasValue());
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetSecureSessionType() == SecureSession::Type::kCASE);
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetPeerNodeId() == kCasePeer2NodeId);
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetLocalNodeId() == kLocalNodeId);
    EXPECT_TRUE(optionalSession.Value()->GetPeer() == ScopedNodeId(kCasePeer2NodeId, kFabricIndex));
    EXPECT_TRUE(optionalSession.Value()->GetLocalScopedNodeId() == ScopedNodeId(kLocalNodeId, kFabricIndex));
    EXPECT_TRUE(optionalSession.Value()->AsSecureSession()->GetLastActivityTime() == 100_ms64);
    peerCATs = optionalSession.Value()->AsSecureSession()->GetPeerCATs();
    EXPECT_TRUE(memcmp(&peerCATs, &kPeer2CATs, sizeof(CATValues)) == 0);
...
TEST_F(TestPeerConnections, TestFindByKeyId)
{
    ...
    EXPECT_TRUE(!connections.FindSecureSessionByLocalKey(1).HasValue());
    ...
    EXPECT_TRUE(!connections.FindSecureSessionByLocalKey(3).HasValue());
