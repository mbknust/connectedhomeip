...
TestContext TestSessionManager::ctx;
...
TEST_F(TestSessionManager, CheckSimpleInitTest)
{
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == fabricTableHolder.Init());
...
TEST_F(TestSessionManager, CheckMessageTest)
{
    ...
    EXPECT_TRUE(!large_buffer.IsNull());
    ...
    ctx.DrainAndServiceIO();
    EXPECT_TRUE(callback.ReceiveHandlerCallCount == 2);
...
TEST_F(TestSessionManager, SendBadEncryptedPacketTest)
{
    ...
    EXPECT_TRUE(badMessageCounterMsg.ExtractPacketHeader(packetHeader) == CHIP_NO_ERROR);

    uint32_t messageCounter = packetHeader.GetMessageCounter();
    packetHeader.SetMessageCounter(messageCounter + 1);
    EXPECT_TRUE(badMessageCounterMsg.InsertPacketHeader(packetHeader) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(badKeyIdMsg.ExtractPacketHeader(packetHeader) == CHIP_NO_ERROR);

    // the secure channel is setup to use key ID 1, and 2. So let's use 3 here.
    packetHeader.SetSessionId(3);
    EXPECT_TRUE(badKeyIdMsg.InsertPacketHeader(packetHeader) == CHIP_NO_ERROR);
...
TEST_F(TestSessionManager, SessionAllocationTest)
{
    ...
    for (int i = 0; i < collisionTestIterations; ++i)
    {
        ...
        for (size_t h = 0; h < numHandles; ++h)
        {
            EXPECT_TRUE(sessionIds[h] != sessionIds[(h + 1) % numHandles]);
        }
        ...
        for (int j = 0; j < UINT16_MAX; ++j)
        {
            ...
            {
                EXPECT_TRUE(potentialCollision != sessionId);
            }
            handle.Value()->AsSecureSession()->MarkForEviction();
        }
...
TEST_F(TestSessionManager, SessionCounterExhaustedTest)
{
    ...
    EXPECT_TRUE(err == CHIP_ERROR_MESSAGE_COUNTER_EXHAUSTED);
...
TEST_F(TestSessionManager, SessionShiftingTest)
{
    ...
    EXPECT_TRUE(session1 != session3);
    EXPECT_TRUE(stickyAliceToBobSession->AsSecureSession() == session1);
    ...
    EXPECT_TRUE(stickyAliceToBobSession->AsSecureSession() == session1);
    EXPECT_TRUE(bobToAliceSession->AsSecureSession() == session2);
    EXPECT_TRUE(aliceToBobSession->AsSecureSession() == session3);
    EXPECT_TRUE(newAliceToBobSession->AsSecureSession() == session3);
