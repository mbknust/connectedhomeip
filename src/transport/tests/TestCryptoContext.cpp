...
class TestCryptoContext : public ::testing::Test
{
public:
...
TEST_F(TestCryptoContext, TestBuildPrivacyNonce)
{
    for (const auto & testVector : thePrivacyNonceTestVector)
    {
        ...
        EXPECT_TRUE(CHIP_NO_ERROR == chip::CryptoContext::BuildPrivacyNonce(privacyNonce, sessionId, mic));
        EXPECT_TRUE(0 == memcmp(privacyNonceView.data(), expectedPrivacyNonce.data(), NONCE_LENGTH));
    }
}
} // namespace