...
TEST(TestGroupMessageCounter, AddPeerTest)
{
    ...
    EXPECT_TRUE(i == CHIP_CONFIG_MAX_GROUP_DATA_PEERS + 1);
    ...
    EXPECT_TRUE(i == CHIP_CONFIG_MAX_FABRICS + 1);
}
...
TEST(TestGroupMessageCounter, RemovePeerTest)
{
    ...
    EXPECT_TRUE(err == CHIP_ERROR_TOO_MANY_PEER_NODES);
    ...
    EXPECT_TRUE(106 == mGroupPeerMsgCounter.GetFabricIndexAt(0));
}
...
TEST(TestGroupMessageCounter, PeerRetrievalTest)
{
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(counter2 != nullptr);
    EXPECT_TRUE(counter2 != counter);
...
TEST(TestGroupMessageCounter, ReorderPeerRemovalTest)
{
    ...
    EXPECT_TRUE(mGroupPeerMsgCounter.GetNodeIdAt(0, 0, true) == 2);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetNodeIdAt(1, 6, false) == 9);

    err = mGroupPeerMsgCounter.RemovePeer(2, 4, false);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetNodeIdAt(1, 3, false) == 8);

    err = mGroupPeerMsgCounter.RemovePeer(2, 1, false);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetNodeIdAt(1, 0, false) == 9);
}
...
TEST(TestGroupMessageCounter, ReorderFabricRemovalTest)
{
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetFabricIndexAt(CHIP_CONFIG_MAX_FABRICS - 1) == kUndefinedFabricIndex);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetFabricIndexAt(2) == CHIP_CONFIG_MAX_FABRICS);
    err = mGroupPeerMsgCounter.RemovePeer(2, 1, false);
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(mGroupPeerMsgCounter.GetFabricIndexAt(1) == CHIP_CONFIG_MAX_FABRICS - 1);
...
TEST(TestGroupMessageCounter, GroupMessageCounterTest)
{
    ...
    EXPECT_TRUE((groupCientCounter.GetCounter(true) - controlCounter) == 1);

    groupCientCounter.SetCounter(true, UINT32_MAX - GROUP_MSG_COUNTER_MIN_INCREMENT);
    EXPECT_TRUE(groupCientCounter.GetCounter(true) == UINT32_MAX - GROUP_MSG_COUNTER_MIN_INCREMENT);

    // Test Persistence
    TestGroupOutgoingCounters groupCientCounter2(&delegate);

    EXPECT_TRUE(groupCientCounter2.GetCounter(true) == UINT32_MAX);
    EXPECT_TRUE((groupCientCounter2.GetCounter(false) - dataCounter) == GROUP_MSG_COUNTER_MIN_INCREMENT);

    // Test Roll over
    groupCientCounter2.IncrementCounter(true);
    EXPECT_TRUE(groupCientCounter2.GetCounter(true) == 0);

    TestGroupOutgoingCounters groupCientCounter3(&delegate);
    EXPECT_TRUE(groupCientCounter3.GetCounter(true) == (UINT32_MAX + GROUP_MSG_COUNTER_MIN_INCREMENT));
    ...
    EXPECT_TRUE((groupCientCounter.GetCounter(false) - dataCounter) == 1);

    groupCientCounter.SetCounter(false, UINT32_MAX - GROUP_MSG_COUNTER_MIN_INCREMENT);
    EXPECT_TRUE(groupCientCounter.GetCounter(false) == UINT32_MAX - GROUP_MSG_COUNTER_MIN_INCREMENT);

    // Test Persistence
    TestGroupOutgoingCounters groupCientCounter4(&delegate);

    EXPECT_TRUE(groupCientCounter4.GetCounter(false) == UINT32_MAX);

    // Test Roll over
    groupCientCounter4.IncrementCounter(false);
    EXPECT_TRUE(groupCientCounter4.GetCounter(false) == 0);

    TestGroupOutgoingCounters groupCientCounter5(&delegate);
    EXPECT_TRUE(groupCientCounter5.GetCounter(false) == (UINT32_MAX + GROUP_MSG_COUNTER_MIN_INCREMENT));
}
