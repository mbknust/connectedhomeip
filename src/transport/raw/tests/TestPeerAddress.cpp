...
TEST(TestPeerAddress, TestPeerAddressMulticast)
{
    ...
    EXPECT_TRUE(chip::Transport::Type::kUdp == addr.GetTransportType());
    EXPECT_TRUE(addr.IsMulticast());

    const Inet::IPAddress & ip = addr.GetIPAddress();
    EXPECT_TRUE(ip.IsIPv6Multicast());
    EXPECT_TRUE(chip::Inet::IPAddressType::kIPv6 == ip.Type());
    ...
    uint8_t * p = result;
    ip.WriteAddress(p);
    EXPECT_TRUE(!memcmp(expected, result, NL_INET_IPV6_ADDR_LEN_IN_BYTES));
}
...
TEST(TestPeerAddress, TestToString)
{
    ...
    IPAddress ip;
    {
        ...
        EXPECT_TRUE(!strcmp(buff, "UDP:[::1]:1122"));
    }

    {
        ...
        EXPECT_TRUE(!strcmp(buff, "TCP:[::1]:1122"));
    }

    {
        PeerAddress::BLE().ToString(buff);
        EXPECT_TRUE(!strcmp(buff, "BLE"));
    }
...

    {
        ...
        udp.SetPort(5840);
        udp.ToString(buff);
        EXPECT_TRUE(!strcmp(buff, "UDP:[::]:5840"));
    }
}
