                          ...
                          CHIP_ERROR error)
{
    ...
    EXPECT_TRUE(result->mTextEntrySize == 1);
    EXPECT_TRUE(strcmp(result->mTextEntries[0].mKey, "key") == 0);
    EXPECT_TRUE(strcmp(reinterpret_cast<const char *>(result->mTextEntries[0].mData), "val") == 0);
...
static void HandleBrowse(void * context, DnssdService * services, size_t servicesSize, bool finalBrowse, CHIP_ERROR error)
{
    ...
    EXPECT_TRUE(ctx->mEndOfInput == false);
    ...
    if (servicesSize > 0)
    {
        ...
        for (unsigned int i = 0; i < servicesSize; i++)
        {
            ...
            EXPECT_TRUE(ChipDnssdResolve(&services[i], services[i].mInterface, HandleResolve, context) == CHIP_NO_ERROR);
        }
    }
}
...
void TestDnssdBrowse_DnssdInitCallback(void * context, CHIP_ERROR error)
{
    ...
    EXPECT_TRUE(ChipDnssdBrowse("_mock", DnssdServiceProtocol::kDnssdProtocolUdp, chip::Inet::IPAddressType::kAny,
                                chip::Inet::InterfaceId::Null(), HandleBrowse, context, &ctx->mBrowseIdentifier) == CHIP_NO_ERROR);
}
...
TEST_F(TestDnssd, TestDnssdBrowse)
{
    ...
    EXPECT_TRUE(server.Listen(chip::DeviceLayer::UDPEndPointManager(), endpoints.get(), 5353) == CHIP_NO_ERROR);

    EXPECT_TRUE(chip::Dnssd::ChipDnssdInit(TestDnssdBrowse_DnssdInitCallback, DnssdErrorCallback, &context) == CHIP_NO_ERROR);
    EXPECT_TRUE(chip::DeviceLayer::SystemLayer().StartTimer(chip::System::Clock::Seconds32(5), Timeout, &context) == CHIP_NO_ERROR);
...
static void TestDnssdPublishService_DnssdInitCallback(void * context, CHIP_ERROR error)
{
    ...
    EXPECT_TRUE(ChipDnssdPublishService(&service, HandlePublish, context) == CHIP_NO_ERROR);

    EXPECT_TRUE(ChipDnssdBrowse("_mock", DnssdServiceProtocol::kDnssdProtocolTcp, chip::Inet::IPAddressType::kAny,
                                chip::Inet::InterfaceId::Null(), HandleBrowse, context, &ctx->mBrowseIdentifier) == CHIP_NO_ERROR);
}
...
TEST_F(TestDnssd, TestDnssdPublishService)
{
    ...
    EXPECT_TRUE(chip::Dnssd::ChipDnssdInit(TestDnssdPublishService_DnssdInitCallback, DnssdErrorCallback, &context) ==
                CHIP_NO_ERROR);
    EXPECT_TRUE(chip::DeviceLayer::SystemLayer().StartTimer(chip::System::Clock::Seconds32(5), Timeout, &context) == CHIP_NO_ERROR);
