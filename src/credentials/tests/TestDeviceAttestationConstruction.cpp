...
class TestDeviceAttestationConstruction : public ::testing::Test
{
public:
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_Roundtrip)
{
    ...
    EXPECT_TRUE(timestamp == timestampDeconstructed);
    EXPECT_TRUE(firmwareInfoSpan.empty());

    EXPECT_TRUE(vendorReservedConstructor.GetNumberOfElements() == vendorReservedDeconstructor.GetNumberOfElements());
    ...
    {
        EXPECT_TRUE(constructionElement->vendorId == deconstructionElement.vendorId);
        EXPECT_TRUE(constructionElement->profileNum == deconstructionElement.profileNum);
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_Deconstruction)
{
    ...
    for (const ByteSpan & attestationElementsTestCase : kTestCases)
    {
        ...
        EXPECT_TRUE(timestampTestVector == timestampDeconstructed);
        EXPECT_TRUE(firmwareInfoDeconstructed.empty());
        EXPECT_TRUE(ArraySize(vendorReservedArrayTestVector) == vendorReserved.GetNumberOfElements());
        struct VendorReservedElement element;

        while (vendorReserved.GetNextVendorReservedElement(element) == CHIP_NO_ERROR)
        {
            EXPECT_TRUE(vendorIdTestVector == element.vendorId);
            EXPECT_TRUE(profileNumTestVector == element.profileNum);
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_DeconstructionNoVendorReserved)
{
    ...
    EXPECT_TRUE(timestampTestVector == timestampDeconstructed);
    EXPECT_TRUE(firmwareInfoDeconstructed.empty());
    EXPECT_TRUE(0 == vendorReserved.GetNumberOfElements());
    struct VendorReservedElement element;

    EXPECT_TRUE(vendorReserved.GetNextVendorReservedElement(element) == CHIP_END_OF_TLV);
}
...
TEST_F(TestDeviceAttestationConstruction, TestVendorReservedData)
{
    ...
    EXPECT_TRUE(i == ArraySize(desiredOrder)); // check if previous loop matched for every array entry.
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_DeconstructionWithFirmwareInfo)
{
    ...
    EXPECT_TRUE(elementsSeen == ArraySize(vendorReservedArrayTestVector));
}
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_DeconstructionUnordered)
{
    ...
    err = DeconstructAttestationElements(ByteSpan(attestationElementsUnorderedTestVector), certificationDeclarationDeconstructed,
                                         attestationNonceDeconstructed, timestampDeconstructed, firmwareInfoDeconstructed,
                                         vendorReserved);
    EXPECT_TRUE(err == CHIP_ERROR_UNEXPECTED_TLV_ELEMENT);
}
...
TEST_F(TestDeviceAttestationConstruction, TestAttestationElements_DeconstructionCorruptedTLV)
{
    ...
    EXPECT_TRUE(err == CHIP_ERROR_TLV_UNDERRUN);
}
...
TEST_F(TestDeviceAttestationConstruction, TestNocsrElements_Construction)
{
    ...
    EXPECT_TRUE(nocsrElements.Get() != nullptr);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(nocsrElementsSpan.size() <= nocsrElementsLen);
