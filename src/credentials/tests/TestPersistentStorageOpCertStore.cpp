...
class TestPersistentStorageOpCertStore : public ::testing::Test
{
public:
...
TEST_F(TestPersistentStorageOpCertStore, TestAddNocFlow)
{
    ...
    EXPECT_TRUE(opCertStore.HasCertificateForFabric(kFabricIndex1, CertChainElement::kRcac) == true); //< From manual add
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 1); //< Storage count did not yet increase
    EXPECT_TRUE(!opCertStore.HasPendingNocChain());
    EXPECT_TRUE(opCertStore.HasPendingRootCert() == true);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 2);

    EXPECT_TRUE(opCertStore.HasCertificateForFabric(kFabricIndex2, CertChainElement::kIcac) == true); //< From manual add
    EXPECT_TRUE(opCertStore.HasCertificateForFabric(kFabricIndex2, CertChainElement::kNoc) == true);  //< From manual add
    ...
    EXPECT_TRUE(opCertStore.HasPendingNocChain());
    EXPECT_TRUE(opCertStore.HasPendingRootCert() == true);
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 0); //< Storage count did not yet increase
    ...
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 3); //< All certs now committed
...
TEST_F(TestPersistentStorageOpCertStore, TestUpdateNocFlow)
{
...

    {
        ...
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        EXPECT_TRUE(storageDelegate.GetNumKeys() == 2);
        EXPECT_TRUE(opCertStore.HasCertificateForFabric(kFabricIndex1, CertChainElement::kIcac) == true); //< From manual add

        err = storageDelegate.SyncSetKeyValue(DefaultStorageKeyAllocator::FabricNOC(kFabricIndex1).KeyName(), kTestNocBufExists,
                                              sizeof(kTestNocBufExists));
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        EXPECT_TRUE(storageDelegate.GetNumKeys() == 3);
        EXPECT_TRUE(opCertStore.HasCertificateForFabric(kFabricIndex1, CertChainElement::kNoc) == true); //< From manual add
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(opCertStore.HasPendingNocChain() == true);
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 3);
    ...
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 1); //< Root remains
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 0); // All keys gone
...
TEST_F(TestPersistentStorageOpCertStore, TestReverts)
{
    ...
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(opCertStore.HasPendingNocChain());
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 3); //< No change to keys
    ...
    // Should see committed root, pending NOC, absent ICAC
    {
        ...
        err       = opCertStore.GetCertificate(kFabricIndex1, CertChainElement::kIcac, largeSpan);
        EXPECT_TRUE(err == CHIP_ERROR_NOT_FOUND);
        EXPECT_TRUE(!opCertStore.HasCertificateForFabric(kFabricIndex1, CertChainElement::kIcac));
    ...
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(!opCertStore.HasPendingNocChain());
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 3); //< Storage count did not yet change
    ...
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(!opCertStore.HasPendingNocChain());
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 2); //< ICAC cert should be gone
...
TEST_F(TestPersistentStorageOpCertStore, TestRevertAddNoc)
{
    ...
    // Add again, and commit
    {
        ...
        EXPECT_TRUE(!opCertStore.HasPendingRootCert());
        EXPECT_TRUE(!opCertStore.HasPendingNocChain());
        EXPECT_TRUE(storageDelegate.GetNumKeys() == 2); //< We have RCAC, NOC, no ICAC
    }
    ...
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(opCertStore.HasPendingNocChain());
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 2); //< No change to keys
    ...
    EXPECT_TRUE(!opCertStore.HasPendingRootCert());
    EXPECT_TRUE(!opCertStore.HasPendingNocChain());
    EXPECT_TRUE(storageDelegate.GetNumKeys() == 3); //< We have RCAC, NOC, ICAC
