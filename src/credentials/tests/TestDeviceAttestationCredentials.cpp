...
class TestDeviceAttestationCredentials : public ::testing::Test
{
public:
...
TEST_F(TestDeviceAttestationCredentials, TestDACProvidersExample_Providers)
{
    ...
    EXPECT_TRUE(default_provider != nullptr);

    CHIP_ERROR err = default_provider->GetDeviceAttestationCert(der_cert_span);
    EXPECT_TRUE(err == CHIP_ERROR_NOT_IMPLEMENTED);
    ...
    EXPECT_TRUE(example_dac_provider != nullptr);
    EXPECT_TRUE(default_provider != example_dac_provider);
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(dac_public_key.Length() == kExpectedDacPublicKey.size());
    EXPECT_TRUE(0 == memcmp(dac_public_key.ConstBytes(), kExpectedDacPublicKey.data(), kExpectedDacPublicKey.size()));
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(pai_public_key.Length() == kExpectedPaiPublicKey.size());
    EXPECT_TRUE(0 == memcmp(pai_public_key.ConstBytes(), kExpectedPaiPublicKey.data(), kExpectedPaiPublicKey.size()));
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(other_data_span.size() > 0);
    EXPECT_TRUE(other_data_span.data()[0] != 0);
...
TEST_F(TestDeviceAttestationCredentials, TestDACProvidersExample_Signature)
{
    ...
    EXPECT_TRUE(out_sig_span.size() == kP256_ECDSA_Signature_Length_Raw);
...
TEST_F(TestDeviceAttestationCredentials, TestDACVerifierExample_AttestationInfoVerification)
{
    ...
    EXPECT_TRUE(default_verifier != nullptr);
    ...
    EXPECT_TRUE(attestationResult == AttestationVerificationResult::kNotImplemented);
    ...
    EXPECT_TRUE(default_verifier != example_dac_verifier);
...
TEST_F(TestDeviceAttestationCredentials, TestDACVerifierExample_CertDeclarationVerification)
{
    ...
    EXPECT_TRUE(attestation_result == AttestationVerificationResult::kSuccess);
...
TEST_F(TestDeviceAttestationCredentials, TestDACVerifierExample_NocsrInformationVerification)
{
    ...
    EXPECT_TRUE(exampleDacVerifier != nullptr);

    P256PublicKey dacPubkey;
    EXPECT_TRUE(sizeof(attestationPublicKey) == dacPubkey.Length());
...
TEST_F(TestDeviceAttestationCredentials, TestAttestationTrustStore)
{
    ...
    EXPECT_TRUE(testAttestationTrustStore != nullptr);
    ...
    for (const auto & testCase : kTestCases)
    {
        ...
        {
            EXPECT_TRUE(paaCertSpan.data_equal(testCase.expectedCertSpan) == true);
        }
    }
}
