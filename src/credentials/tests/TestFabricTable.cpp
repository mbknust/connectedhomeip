                                           ...
                                           FabricTable::AdvertiseIdentity advertiseIdentity = FabricTable::AdvertiseIdentity::Yes)
{
    ...
    EXPECT_TRUE(opKeysSerialized.SetLength(TestCerts::sTestCert_Node02_01_PublicKey.size() +
                                           TestCerts::sTestCert_Node02_01_PrivateKey.size()) == CHIP_NO_ERROR);
    EXPECT_TRUE(opKey_Node02_01.Deserialize(opKeysSerialized) == CHIP_NO_ERROR);

    EXPECT_TRUE(fabricTable.AddNewPendingTrustedRootCert(rcacSpan) == CHIP_NO_ERROR);
...
class TestFabricTable : public ::testing::Test
{
public:
...
TEST_F(TestFabricTable, TestLastKnownGoodTimeInit)
{
    ...
    EXPECT_TRUE(fabricTableHolder.Init(&testStorage) == CHIP_NO_ERROR);
    System::Clock::Seconds32 lastKnownGoodChipEpochTime;

    FabricTable & fabricTable = fabricTableHolder.GetFabricTable();
    EXPECT_TRUE(fabricTable.GetLastKnownGoodChipEpochTime(lastKnownGoodChipEpochTime) == CHIP_NO_ERROR);
    System::Clock::Seconds32 firmwareBuildTime;
    EXPECT_TRUE(DeviceLayer::ConfigurationMgr().GetFirmwareBuildChipEpochTime(firmwareBuildTime) == CHIP_NO_ERROR);
    EXPECT_TRUE(lastKnownGoodChipEpochTime == firmwareBuildTime);
}
...
TEST_F(TestFabricTable, TestCollidingFabrics)
{
    ...
    //
    EXPECT_TRUE(LoadTestFabric_Node01_01(fabricTable, /* doCommit = */ true) == CHIP_NO_ERROR);
    EXPECT_TRUE(LoadTestFabric_Node01_02(fabricTable, /* doCommit = */ true) != CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(Credentials::ExtractPublicKeyFromChipCert(rcacSpan, rootPublicKeySpan) == CHIP_NO_ERROR);
    ...
    //
    {
        ...
        EXPECT_TRUE(ExtractNodeIdFabricIdFromOpCert(origNocSpan, &nodeId, &fabricId) == CHIP_NO_ERROR);
        EXPECT_TRUE(fabricTable.FindIdentity(rootPublicKeySpan, fabricId, nodeId) != nullptr);
    }
...
TEST_F(TestFabricTable, TestUpdateLastKnownGoodTime)
{
    ...
    EXPECT_TRUE(Credentials::ASN1ToChipEpochTime(asn1Expected, testCertNotBeforeSeconds) == CHIP_NO_ERROR);
    ...
    for (auto buildTime : afterNotBeforeBuildTimes)
    {
        ...
        EXPECT_TRUE(DeviceLayer::ConfigurationMgr().SetFirmwareBuildChipEpochTime(buildTime) == CHIP_NO_ERROR);
        chip::TestPersistentStorageDelegate testStorage;

        {
            ...
            EXPECT_TRUE(fabricTable.GetLastKnownGoodChipEpochTime(lastKnownGoodTime) == CHIP_NO_ERROR);
            EXPECT_TRUE(lastKnownGoodTime == buildTime);

            // Load a test fabric, but do not commit.
            EXPECT_TRUE(LoadTestFabric_Node01_01(fabricTable, /* doCommit = */ false) == CHIP_NO_ERROR);
    ...
    for (auto buildTime : beforeNotBeforeBuildTimes)
    {
        ...
        chip::TestPersistentStorageDelegate testStorage;
        {
            ...
            System::Clock::Seconds32 lastKnownGoodTime;
            EXPECT_TRUE(fabricTable.GetLastKnownGoodChipEpochTime(lastKnownGoodTime) == CHIP_NO_ERROR);
            EXPECT_TRUE(lastKnownGoodTime == testCertNotBeforeTime);
...
TEST_F(TestFabricTable, TestSetLastKnownGoodTime)
{
    ...
    for (auto buildTime : testCaseFirmwareBuildTimes)
    {
        ...
        System::Clock::Seconds32 newTime;
        {
            ...
            System::Clock::Seconds32 lastKnownGoodTime;
            EXPECT_TRUE(fabricTable.GetLastKnownGoodChipEpochTime(lastKnownGoodTime) == CHIP_NO_ERROR);
            EXPECT_TRUE(lastKnownGoodTime == initialLastKnownGoodTime);
            ...
            EXPECT_TRUE(fabricTable.SetLastKnownGoodChipEpochTime(newTime) != CHIP_NO_ERROR);
            ...
            EXPECT_TRUE(fabricTable.SetLastKnownGoodChipEpochTime(lastKnownGoodTime) == CHIP_NO_ERROR);
            ...
            EXPECT_TRUE(fabricTable.GetLastKnownGoodChipEpochTime(lastKnownGoodTime) == CHIP_NO_ERROR);
            EXPECT_TRUE(lastKnownGoodTime == newTime);
        }
        {
...
TEST_F(TestFabricTable, TestBasicAddNocUpdateNocFlow)
{
    ...
    EXPECT_TRUE(fabric11Node55Keypair.Initialize(Crypto::ECPKeyTarget::ECDSA) == CHIP_NO_ERROR);

    // Initialize a fabric table.
    ScopedFabricTable fabricTableHolder;
    EXPECT_TRUE(fabricTableHolder.Init(&storage) == CHIP_NO_ERROR);
    FabricTable & fabricTable = fabricTableHolder.GetFabricTable();

    ASSERT_EQ(fabricTable.FabricCount(), 0);

    {
        FabricIndex nextFabricIndex = kUndefinedFabricIndex;
        EXPECT_TRUE(fabricTable.PeekFabricIndexForNextAddition(nextFabricIndex) == CHIP_NO_ERROR);
        ASSERT_EQ(nextFabricIndex, 1);
    }
    ...
    // Sequence 1: Add node ID 55 on fabric 11, using externally owned key and no ICAC --> Yield fabricIndex 1
    {
        ...
        // Validate iterator sees nothing yet
        {
            ...
            EXPECT_TRUE(numFabricsIterated == 0);
            EXPECT_TRUE(saw1 == false);
        }
        ...
        uint8_t rcacBuf[Credentials::kMaxCHIPCertLength];
        {
            ...
            EXPECT_TRUE(fabricTable.FetchPendingNonFabricAssociatedRootCert(fetchedSpan) == CHIP_ERROR_NOT_FOUND);
        }

        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AddNewPendingTrustedRootCert(rcac));
        {
            // Now have a pending root cert.
            MutableByteSpan fetchedSpan{ rcacBuf };
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchPendingNonFabricAssociatedRootCert(fetchedSpan));
        ...
        // No storage yet
        EXPECT_TRUE(storage.GetNumKeys() == numStorageKeysAtStart);
        ...
        // Validate iterator sees pending
        {
            ...
            for (const auto & iterFabricInfo : fabricTable)
            {
                ...
                if (iterFabricInfo.GetFabricIndex() == 1)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == nodeId);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == fabricId);
                    saw1 = true;
                }
            }

            EXPECT_TRUE(numFabricsIterated == 1);
            EXPECT_TRUE(saw1 == true);
        }

        // Commit, now storage should have keys
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.CommitPendingFabricData());

        EXPECT_TRUE(storage.GetNumKeys() == (numStorageKeysAtStart + 4)); // 2 opcerts + fabric metadata + index

        // Next fabric index has been updated.
        {
            FabricIndex nextFabricIndex = kUndefinedFabricIndex;
            EXPECT_TRUE(fabricTable.PeekFabricIndexForNextAddition(nextFabricIndex) == CHIP_NO_ERROR);
            ASSERT_EQ(nextFabricIndex, 2);
        }

        // Validate contents
        const auto * fabricInfo = fabricTable.FindFabricWithIndex(1);
        EXPECT_TRUE(fabricInfo != nullptr);
        if (fabricInfo != nullptr)
        {
            Credentials::ChipCertificateSet certificates;
            ASSERT_EQ(CHIP_NO_ERROR, certificates.Init(1));
            ASSERT_EQ(CHIP_NO_ERROR, certificates.LoadCert(rcac, BitFlags<CertDecodeFlags>(CertDecodeFlags::kIsTrustAnchor)));
            Crypto::P256PublicKey rcacPublicKey(certificates.GetCertSet()[0].mPublicKey);

            EXPECT_TRUE(fabricInfo->GetFabricIndex() == newFabricIndex);
            EXPECT_TRUE(fabricInfo->GetNodeId() == nodeId);
            EXPECT_TRUE(fabricInfo->GetFabricId() == fabricId);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);

            Crypto::P256PublicKey rootPublicKeyOfFabric;
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootPubkey(newFabricIndex, rootPublicKeyOfFabric));
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(newFabricIndex, ByteSpan{ message }, sig));
        ASSERT_EQ(CHIP_NO_ERROR, fabric11Node55Keypair.Pubkey().ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
    ...
    // Sequence 2: Add node ID 999 on fabric 44, using operational keystore and ICAC --> Yield fabricIndex 2
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AllocatePendingOperationalKey(chip::NullOptional, csrSpan));
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AddNewPendingFabricWithOperationalKeystore(noc, icac, kVendorId, &newFabricIndex));
        ASSERT_EQ(fabricTable.FabricCount(), 2);
        EXPECT_TRUE(newFabricIndex == 2);
        // No storage yet
        EXPECT_TRUE(storage.GetNumKeys() == numStorageAfterFirstAdd);
        // Next fabric index has not been updated yet.
        {
            FabricIndex nextFabricIndex = kUndefinedFabricIndex;
            EXPECT_TRUE(fabricTable.PeekFabricIndexForNextAddition(nextFabricIndex) == CHIP_NO_ERROR);
        ...
        // Validate that fabric has the correct operational key by verifying a signature
        {
            ...
            ASSERT_EQ(CHIP_NO_ERROR, VerifyCertificateSigningRequest(csrSpan.data(), csrSpan.size(), nocPubKey));

            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(newFabricIndex, ByteSpan{ message }, sig));
            ASSERT_EQ(CHIP_NO_ERROR, nocPubKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
        }
        ...
        // Verify we can now see 2 fabrics with the iterator
        {
            ...
            for (const auto & iterFabricInfo : fabricTable)
            {
                ...
                if (iterFabricInfo.GetFabricIndex() == 1)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 55);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 11);
                    saw1 = true;
                }
                if (iterFabricInfo.GetFabricIndex() == 2)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 999);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 44);
                    saw2 = true;
                }
            }

            EXPECT_TRUE(numFabricsIterated == 2);
            EXPECT_TRUE(saw1 == true);
            EXPECT_TRUE(saw2 == true);
        }
    }
    ...
    // Sequence 3: Update node ID 999 to 1000 on fabric 44, using operational keystore and no ICAC --> Stays fabricIndex 2
    {
        ...
        // No storage yet
        EXPECT_TRUE(storage.GetNumKeys() == numStorageAfterSecondAdd);
        ...
        // Validate iterator sees the pending data
        {
            ...
            for (const auto & iterFabricInfo : fabricTable)
            {
                ...
                if (iterFabricInfo.GetFabricIndex() == 2)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 1000);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 44);
                    EXPECT_TRUE(!iterFabricInfo.ShouldAdvertiseIdentity());
                    saw2 = true;
                }
        ...
        ASSERT_EQ(storage.GetNumKeys(), (numStorageAfterSecondAdd - 1)); // ICAC got deleted
        ...
        if (fabricInfo != nullptr)
        {
            ...
            EXPECT_TRUE(fabricInfo->GetFabricIndex() == fabricIndex);
            EXPECT_TRUE(fabricInfo->GetNodeId() == nodeId);
            EXPECT_TRUE(fabricInfo->GetFabricId() == fabricId);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);
            ...
            Crypto::P256PublicKey rootPublicKeyOfFabric;
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootPubkey(fabricIndex, rootPublicKeyOfFabric));
            EXPECT_TRUE(rootPublicKeyOfFabric.Matches(rcacPublicKey));
        }
        ...
        // Validate that fabric has the correct operational key by verifying a signature
        {
            ...
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(fabricIndex, ByteSpan{ message }, sig));
            ASSERT_EQ(CHIP_NO_ERROR, nocPubKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
        }
    ...
    {
        ASSERT_EQ(fabricTable.FabricCount(), 2);
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SetFabricLabel(2, CharSpan("roboto")));
        ASSERT_EQ(fabricTable.FabricCount(), 2);

        ASSERT_EQ(storage.GetNumKeys(), numStorageAfterUpdate); // Number of keys unchanged
        ...
        // Validate basic contents
        {
            ...
            if (fabricInfo != nullptr)
            {
                EXPECT_TRUE(fabricInfo->GetFabricIndex() == 2);
                EXPECT_TRUE(fabricInfo->GetNodeId() == 1000);
                EXPECT_TRUE(fabricInfo->GetFabricId() == 44);
                EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
    ...
    // Sequence 5: Remove FabricIndex 1 (FabricId 11, NodeId 55), make sure FabricIndex 2 (FabricId 44, NodeId 1000) still exists
    {
        ...
        {
            ASSERT_EQ(fabricTable.FabricCount(), 2);
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.Delete(1));
            ASSERT_EQ(fabricTable.FabricCount(), 1);

            ASSERT_EQ(storage.GetNumKeys(), (numStorageAfterUpdate - 3)); // Deleted NOC, RCAC, Metadata
        }
        ...
        if (fabricInfo != nullptr)
        {
            ...
            Crypto::P256PublicKey rootPublicKeyOfFabric;
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootPubkey(2, rootPublicKeyOfFabric));
        }
        ...
        // Validate that fabric has the correct operational key by verifying a signature
        {
            ...
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchNOCCert(2, nocSpan));

            Credentials::ChipCertificateSet certificates;
            ASSERT_EQ(CHIP_NO_ERROR, certificates.Init(1));
            ASSERT_EQ(CHIP_NO_ERROR, certificates.LoadCert(nocSpan, BitFlags<CertDecodeFlags>(CertDecodeFlags::kIsTrustAnchor)));
            Crypto::P256PublicKey nocPubKey(certificates.GetCertSet()[0].mPublicKey);

            Crypto::P256ECDSASignature sig;
            uint8_t message[] = { 'm', 's', 'g' };

            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(2, ByteSpan{ message }, sig));
            ASSERT_EQ(CHIP_NO_ERROR, nocPubKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
        }
...
TEST_F(TestFabricTable, TestAddMultipleSameRootDifferentFabricId)
{
    ...
    // First scope: add FabricID 1111, node ID 55
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricCertAuthority.SetIncludeIcac(true).GenerateNocChain(fabricId, nodeId, csrSpan).GetStatus());
        ...
        if (fabricInfo != nullptr)
        {
            EXPECT_TRUE(fabricInfo->GetFabricIndex() == 1);
            EXPECT_TRUE(fabricInfo->GetNodeId() == 55);
            EXPECT_TRUE(fabricInfo->GetFabricId() == 1111);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);
        }
    }
    size_t numStorageKeysAfterFirstAdd = storage.GetNumKeys();
    EXPECT_TRUE(numStorageKeysAfterFirstAdd == 7); // Metadata, index, 3 certs, 1 opkey, last known good time
    ...
    // Second scope: add FabricID 2222, node ID 66, same root as first
    {
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AddNewPendingTrustedRootCert(rcac2));
        FabricIndex newFabricIndex = kUndefinedFabricIndex;
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AddNewPendingFabricWithOperationalKeystore(noc, icac, kVendorId, &newFabricIndex));
        ASSERT_EQ(fabricTable.FabricCount(), 2);
        EXPECT_TRUE(newFabricIndex == 2);
        ...
        {
            EXPECT_TRUE(fabricInfo->GetFabricIndex() == 2);
            EXPECT_TRUE(fabricInfo->GetNodeId() == 66);
            EXPECT_TRUE(fabricInfo->GetFabricId() == 2222);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);
        }
    }
    size_t numStorageKeysAfterSecondAdd = storage.GetNumKeys();
    EXPECT_TRUE(numStorageKeysAfterSecondAdd == (numStorageKeysAfterFirstAdd + 5)); // Add 3 certs, 1 metadata, 1 opkey
}
...
TEST_F(TestFabricTable, TestAddMultipleSameFabricIdDifferentRoot)
{
    ...
    // First scope: add FabricID 1111, node ID 55
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricCertAuthority1.SetIncludeIcac(true).GenerateNocChain(fabricId, nodeId, csrSpan).GetStatus());
    ...
    // Second scope: add FabricID 1111, node ID 66, different root from first
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricCertAuthority2.SetIncludeIcac(true).GenerateNocChain(fabricId, nodeId, csrSpan).GetStatus());
...
TEST_F(TestFabricTable, TestPersistence)
{
    ...
    // First scope: add 2 fabrics with same root: (1111, 2222), commit them, keep track of public keys
    {
        ...
        // Add Fabric 1111 Node Id 55
        {
            ...
            // Validate that fabric has the correct operational key by verifying a signature
            {
                ...
                ASSERT_EQ(CHIP_NO_ERROR, VerifyCertificateSigningRequest(csrSpan.data(), csrSpan.size(), fIdx1PublicKey));

                ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(newFabricIndex, ByteSpan{ message }, sig));
                ASSERT_EQ(CHIP_NO_ERROR, fIdx1PublicKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
            }
        }
        ...
        // Add Fabric 2222 Node Id 66, no ICAC
        {
            ...
            // Validate that fabric has the correct operational key by verifying a signature
            {
                ...
                ASSERT_EQ(CHIP_NO_ERROR, VerifyCertificateSigningRequest(csrSpan.data(), csrSpan.size(), fIdx2PublicKey));

                ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(newFabricIndex, ByteSpan{ message }, sig));
                ASSERT_EQ(CHIP_NO_ERROR, fIdx2PublicKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
            }
        }

        EXPECT_TRUE(fabricTable.FabricCount() == 2);
        ...
        // Verify we can now see 2 fabrics with the iterator
        {
            ...
            for (const auto & iterFabricInfo : fabricTable)
            {
                ...
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 55);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 1111);
                    saw1 = true;
                }
                if (iterFabricInfo.GetFabricIndex() == 2)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 66);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 2222);
                    saw2 = true;
                }
            }
    ...
    EXPECT_TRUE(storage.GetNumKeys() == (2 + 5 + 4));
    ...
    // Second scope: Validate that a fresh FabricTable loads the previously committed fabrics on Init.
    {
        ...
        // Validate contents of Fabric 2222
        {
            ...
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootCert(2, rcacSpan));
        ...
        // Validate contents of Fabric 1111
        {
            uint8_t rcacBuf[Credentials::kMaxCHIPCertLength];
            MutableByteSpan rcacSpan{ rcacBuf };
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootCert(1, rcacSpan));
            ...
            if (fabricInfo != nullptr)
            {
                ...
                Crypto::P256PublicKey rootPublicKeyOfFabric;
                ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchRootPubkey(1, rootPublicKeyOfFabric));
                EXPECT_TRUE(rootPublicKeyOfFabric.Matches(rcacPublicKey));
            }
            ...
            // Validate that fabric has the correct operational key by verifying a signature
            {
                ...
                ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(1, ByteSpan{ message }, sig));
                ASSERT_EQ(CHIP_NO_ERROR, fIdx1PublicKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig));
            }
            ...
            // Validate that signing with Fabric index 2 fails to verify with fabric index 1
            {
                ...
                ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SignWithOpKeypair(2, ByteSpan{ message }, sig));
                EXPECT_TRUE(fIdx1PublicKey.ECDSA_validate_msg_signature(&message[0], sizeof(message), sig) ==
...
TEST_F(TestFabricTable, TestAddNocFailSafe)
{
    ...
    // Sequence 1: Add node ID 55 on fabric 11, see that pending works, and that revert works
    {
        ...
        // No storage yet
        EXPECT_TRUE(storage.GetNumKeys() == numStorageKeysAtStart); // Nothing yet
        ...
        EXPECT_TRUE(storage.GetNumKeys() == (numStorageKeysAtStart + 1));
    ...
    // Sequence 2: Add node ID 999 on fabric 44, using operational keystore and ICAC --> Yield fabricIndex 1
    {
        ...
        EXPECT_TRUE(newFabricIndex == 1);
        // No storage yet
        EXPECT_TRUE(storage.GetNumKeys() == numStorageAfterRevert);
...

    {
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        EXPECT_TRUE(storage.GetNumKeys() == numStorageAfterAdd);
...
TEST_F(TestFabricTable, TestUpdateNocFailSafe)
{
    ...
    // Sequence 2: Do an Update to NodeId 1000, with no ICAC, but revert it
    {
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.UpdatePendingFabricWithOperationalKeystore(1, noc, ByteSpan{}));
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ...
        ASSERT_EQ(storage.GetNumKeys(), numStorageAfterAdd);
        ...
        if (fabricInfo != nullptr)
        {
            ...
            EXPECT_TRUE(fabricInfo->GetFabricIndex() == fabricIndex);
            EXPECT_TRUE(fabricInfo->GetNodeId() == 999);
            EXPECT_TRUE(fabricInfo->GetFabricId() == 44);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);
        ...
        // Validate that fabric has the correct operational key by verifying a signature
        {
            uint8_t nocBuf[Credentials::kMaxCHIPCertLength];
            MutableByteSpan nocSpan{ nocBuf };
            ASSERT_EQ(CHIP_NO_ERROR, fabricTable.FetchNOCCert(1, nocSpan));
    ...
    // Sequence 3: Do an Update to NodeId 1001, with no ICAC, but commit it
    {
        ...
        // Validate iterator sees the pending data
        {
            ...
            for (const auto & iterFabricInfo : fabricTable)
            {
                ...
                if (iterFabricInfo.GetFabricIndex() == 1)
                {
                    EXPECT_TRUE(iterFabricInfo.GetNodeId() == 1001);
                    EXPECT_TRUE(iterFabricInfo.GetFabricId() == 44);
                    EXPECT_TRUE(iterFabricInfo.ShouldAdvertiseIdentity());
                    saw1 = true;
                }
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ASSERT_EQ(storage.GetNumKeys(), numStorageAfterAdd - 1);
        ...
        if (fabricInfo != nullptr)
        {
            ...
            EXPECT_TRUE(fabricInfo->GetFabricIndex() == fabricIndex);
            EXPECT_TRUE(fabricInfo->GetNodeId() == 1001);
            EXPECT_TRUE(fabricInfo->GetFabricId() == 44);
            EXPECT_TRUE(fabricInfo->GetVendorId() == kVendorId);
            EXPECT_TRUE(fabricInfo->GetFabricLabel().size() == 0);
...
TEST_F(TestFabricTable, TestFabricLabelChange)
{
    ...
    // Second scope: set FabricLabel to "acme fabric", make sure it cannot be reverted
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.GetFabricLabel(1, fabricLabel));
        EXPECT_TRUE(fabricLabel.size() == 0);

        // Set a valid name
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SetFabricLabel(1, CharSpan::fromCharString("acme fabric")));
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.GetFabricLabel(1, fabricLabel));
        EXPECT_TRUE(fabricLabel.data_equal(CharSpan::fromCharString("acme fabric")) == true);
        ...
        EXPECT_TRUE(fabricLabelTooBig.size() > chip::kFabricLabelMaxLengthInBytes);

        EXPECT_TRUE(fabricTable.SetFabricLabel(1, fabricLabelTooBig) == CHIP_ERROR_INVALID_ARGUMENT);
    ...
    // Third scope: set fabric label after an update, it sticks, but then goes back after revert
    {
        ...
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.UpdatePendingFabricWithOperationalKeystore(1, noc, icac));
        ASSERT_EQ(fabricTable.FabricCount(), 1);
        ...
        // Update fabric label
        CharSpan fabricLabel = CharSpan::fromCharString("placeholder");
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.SetFabricLabel(1, CharSpan::fromCharString("roboto fabric")));

        fabricLabel = CharSpan::fromCharString("placeholder");
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.GetFabricLabel(1, fabricLabel));
        EXPECT_TRUE(fabricLabel.data_equal(CharSpan::fromCharString("roboto fabric")) == true);
...
TEST_F(TestFabricTable, TestFabricLookup)
{
    ...
    FabricTable & fabricTable = fabricTableHolder.GetFabricTable();
    EXPECT_TRUE(LoadTestFabric_Node01_01(fabricTable, /* doCommit = */ true) == CHIP_NO_ERROR);
    EXPECT_TRUE(LoadTestFabric_Node02_01(fabricTable, /* doCommit = */ true, FabricTable::AdvertiseIdentity::No) == CHIP_NO_ERROR);

    // Attempt lookup of the Root01 fabric.
    {
        Crypto::P256PublicKey key;
        EXPECT_TRUE(key.Length() >= TestCerts::sTestCert_Root01_PublicKey.size());
    ...
    {
        Crypto::P256PublicKey key;
        EXPECT_TRUE(key.Length() >= TestCerts::sTestCert_Root02_PublicKey.size());
    ...
    {
        EXPECT_TRUE(fabricTable.FindFabricWithIndex(0) == nullptr);
    }
}
...
TEST_F(TestFabricTable, TestFetchCATs)
{
    ...
    FabricTable & fabricTable = fabricTableHolder.GetFabricTable();
    EXPECT_TRUE(LoadTestFabric_Node01_01(fabricTable, /* doCommit = */ true) == CHIP_NO_ERROR);
    EXPECT_TRUE(LoadTestFabric_Node02_01(fabricTable, /* doCommit = */ true) == CHIP_NO_ERROR);

    // Attempt Fetching fabric index 1 CATs and verify contents.
    {
        CATValues cats;
        EXPECT_TRUE(fabricTable.FetchCATs(1, cats) == CHIP_NO_ERROR);
        // Test fabric NOCs don't contain any CATs.
        EXPECT_TRUE(cats == kUndefinedCATs);
    }

    // Attempt Fetching fabric index 2 CATs and verify contents.
    {
        CATValues cats;
        EXPECT_TRUE(fabricTable.FetchCATs(2, cats) == CHIP_NO_ERROR);
        // Test fabric NOCs don't contain any CATs.
        EXPECT_TRUE(cats == kUndefinedCATs);
    }
...
TEST_F(TestFabricTable, TestAddNocRootCollision)
{
    ...
    // Second scope: add FabricID 1111, node ID 55 *again* --> Collision of Root/FabricID with existing
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AddNewPendingTrustedRootCert(rcac));
        FabricIndex newFabricIndex = kUndefinedFabricIndex;
        EXPECT_TRUE(fabricTable.AddNewPendingFabricWithOperationalKeystore(noc, icac, kVendorId, &newFabricIndex) ==
    ...
    EXPECT_TRUE(storage.GetNumKeys() == numStorageKeysAfterFirstAdd);
    ...
    EXPECT_TRUE(numStorageKeysAfterSecondAdd == (numStorageKeysAfterFirstAdd + 5)); // Metadata, 3 certs, 1 opkey
}
...
TEST_F(TestFabricTable, TestInvalidChaining)
{
    ...
    // then succeed with proper chaining
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.AllocatePendingOperationalKey(chip::NullOptional, csrSpan));
        ASSERT_EQ(CHIP_NO_ERROR, fabricCertAuthority.SetIncludeIcac(false).GenerateNocChain(fabricId, nodeId, csrSpan).GetStatus());
        ASSERT_EQ(CHIP_NO_ERROR,
                  differentCertAuthority.SetIncludeIcac(false).GenerateNocChain(fabricId, nodeId, csrSpan).GetStatus());
...
TEST_F(TestFabricTable, TestEphemeralKeys)
{
    ...
    // Use a fabric table without an operational keystore: should still work
    {
        ...
        ASSERT_EQ(CHIP_NO_ERROR, opCertStore.Init(&storage));
        ...
        ASSERT_EQ(CHIP_NO_ERROR, fabricTable.Init(initParams));
...
TEST_F(TestFabricTable, TestCommitMarker)
{
    ...
    //      - Abort commit on second fabric
    {
        ...
        EXPECT_TRUE(fabricTable.GetDeletedFabricFromCommitMarker() == kUndefinedFabricIndex);
        ASSERT_EQ(fabricTable.FabricCount(), 0);
        ...
        // Add Fabric 2222 Node Id 66, no ICAC *** AND ABORT COMMIT ***
        {
            ...
            EXPECT_TRUE(fabricTable.CommitPendingFabricData() == CHIP_ERROR_INTERNAL);
            ...
            EXPECT_TRUE(storage.GetNumKeys() > (numStorageKeysAfterFirstAdd + 1));
        }
...
#if CONFIG_BUILD_FOR_HOST_UNIT_TEST
    {
        ...
        EXPECT_TRUE(fabricTable.GetDeletedFabricFromCommitMarker() == 2);
