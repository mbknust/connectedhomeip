...
TEST_F(TestExtensionFieldSets, TestInsertExtensionFieldSet)
{
    ...
    EXPECT_TRUE(true == EFS->IsEmpty());

    // Test creators of single ExtensionFieldSet
    EXPECT_TRUE(EFS1.mID == kOnOffClusterId);
    EXPECT_TRUE(EFS1.mUsedBytes == kOnOffSize);
    EXPECT_TRUE(!memcmp(onOffBuffer, EFS1.mBytesBuffer, EFS1.mUsedBytes));

    EXPECT_TRUE(EFS2.mID == kLevelControlClusterId);
    EXPECT_TRUE(EFS2.mUsedBytes == kLevelControlSize);
    EXPECT_TRUE(!memcmp(levelControlBuffer, EFS2.mBytesBuffer, EFS2.mUsedBytes));

    EXPECT_TRUE(EFS3.mID == kColorControlClusterId);
    EXPECT_TRUE(EFS3.mUsedBytes == kColorControlSize);
    EXPECT_TRUE(!memcmp(colorControlBuffer, EFS3.mBytesBuffer, EFS3.mUsedBytes));

    // operator tests single EFS
    tempEFS = EFS1;
    EXPECT_TRUE(tempEFS == EFS1);
    tempEFS = EFS2;
    EXPECT_TRUE(tempEFS == EFS2);
    tempEFS = EFS3;
    EXPECT_TRUE(tempEFS == EFS3);

    // Test clear EFS
    tempEFS.Clear();
    EXPECT_TRUE(tempEFS.IsEmpty());
    EXPECT_TRUE(tempEFS.mID == kInvalidClusterId);
    EXPECT_TRUE(tempEFS.mUsedBytes == 0);
    EXPECT_TRUE(!memcmp(empty_buffer, tempEFS.mBytesBuffer, sizeof(tempEFS.mBytesBuffer)));
    ...
    EXPECT_TRUE(tempEFS.mID == kOnOffClusterId);
    // Confirm EFS empty
    EXPECT_TRUE(tempEFS.mUsedBytes == 0);
    EXPECT_TRUE(!memcmp(empty_buffer, tempEFS.mBytesBuffer, sizeof(empty_buffer)));

    tempEFS = scenes::ExtensionFieldSet(kLevelControlClusterId, bufferSpan);
    EXPECT_TRUE(tempEFS.mID == kLevelControlClusterId);
    // Confirm EFS empty
    EXPECT_TRUE(tempEFS.mUsedBytes == 0);
    EXPECT_TRUE(!memcmp(empty_buffer, tempEFS.mBytesBuffer, sizeof(empty_buffer)));
    ...
    EXPECT_TRUE(tempEFS.mID == kColorControlClusterId);
    // Confirm EFS was written
    EXPECT_TRUE(tempEFS.mUsedBytes == static_cast<uint8_t>(sizeof(tempEFS.mBytesBuffer)));
    EXPECT_TRUE(!memcmp(double_size_buffer, tempEFS.mBytesBuffer, sizeof(tempEFS.mBytesBuffer)));

    tempEFS.Clear();
    EXPECT_TRUE(tempEFS.IsEmpty());

    // Test insertion of uninitialized EFS
    EXPECT_TRUE(CHIP_ERROR_INVALID_ARGUMENT == EFS->InsertFieldSet(tempEFS));
    EXPECT_TRUE(0 == EFS->GetFieldSetCount());
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == testEFS1.InsertFieldSet(EFS1));
    EXPECT_TRUE(CHIP_NO_ERROR == testEFS1.InsertFieldSet(EFS2));
    EXPECT_TRUE(CHIP_NO_ERROR == testEFS1.InsertFieldSet(EFS3));

    EXPECT_TRUE(CHIP_NO_ERROR == testEFS2.InsertFieldSet(EFS3));

    EXPECT_TRUE(CHIP_NO_ERROR == testEFS3.InsertFieldSet(EFS1));
    EXPECT_TRUE(CHIP_NO_ERROR == testEFS3.InsertFieldSet(EFS2));

    tempTestEFS = testEFS1;
    EXPECT_TRUE(tempTestEFS == testEFS1);
    EXPECT_TRUE(!(tempTestEFS == testEFS2));
    EXPECT_TRUE(!(tempTestEFS == testEFS3));
    ...
    EXPECT_TRUE(0 == tempTestEFS.GetFieldSetCount());

    // Test insert
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS1));
    EXPECT_TRUE(1 == EFS->GetFieldSetCount());

    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS2));
    EXPECT_TRUE(2 == EFS->GetFieldSetCount());

    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS3));
    EXPECT_TRUE(3 == EFS->GetFieldSetCount());

    // Test get
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->GetFieldSetAtPosition(tempEFS, 0));
    EXPECT_TRUE(tempEFS == EFS1);

    EXPECT_TRUE(CHIP_NO_ERROR == EFS->GetFieldSetAtPosition(tempEFS, 1));
    EXPECT_TRUE(tempEFS == EFS2);

    EXPECT_TRUE(CHIP_NO_ERROR == EFS->GetFieldSetAtPosition(tempEFS, 2));
    EXPECT_TRUE(tempEFS == EFS3);
}
...
TEST_F(TestExtensionFieldSets, TestSerializeDerializeExtensionFieldSet)
{
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == EFS1.Serialize(writer));
    EFS1_serialized_length = writer.GetLengthWritten();
    EXPECT_TRUE(EFS1_serialized_length <= scenes::kMaxFieldBytesPerCluster);

    writer.Init(EFS2Buffer);
    EXPECT_TRUE(CHIP_NO_ERROR == EFS2.Serialize(writer));
    EFS2_serialized_length = writer.GetLengthWritten();
    EXPECT_TRUE(EFS2_serialized_length <= scenes::kMaxFieldBytesPerCluster);

    writer.Init(EFS3Buffer);
    EXPECT_TRUE(CHIP_NO_ERROR == EFS3.Serialize(writer));
    EFS3_serialized_length = writer.GetLengthWritten();
    EXPECT_TRUE(EFS3_serialized_length <= scenes::kMaxFieldBytesPerCluster);

    reader.Init(EFS1Buffer);
    reader.Next(TLV::AnonymousTag());
    EXPECT_TRUE(CHIP_NO_ERROR == tempEFS.Deserialize(reader));
    EXPECT_TRUE(EFS1 == tempEFS);

    reader.Init(EFS2Buffer);
    reader.Next(TLV::AnonymousTag());
    EXPECT_TRUE(CHIP_NO_ERROR == tempEFS.Deserialize(reader));
    EXPECT_TRUE(EFS2 == tempEFS);

    reader.Init(EFS3Buffer);
    reader.Next(TLV::AnonymousTag());
    EXPECT_TRUE(CHIP_NO_ERROR == tempEFS.Deserialize(reader));
    EXPECT_TRUE(EFS3 == tempEFS);
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->Serialize(writer, TLV::ContextTag(TagTestEFS::kEFS)));
    writer.EndContainer(outer);
    sceneEFS_serialized_length = writer.GetLengthWritten();
    EXPECT_TRUE(sceneEFS_serialized_length <= kPersistentSceneBufferMax);

    reader.Init(sceneEFSBuffer);
    EXPECT_TRUE(CHIP_NO_ERROR == reader.Next());
    EXPECT_TRUE(CHIP_NO_ERROR == reader.EnterContainer(outerRead));
    EXPECT_TRUE(CHIP_NO_ERROR == testSceneEFS.Deserialize(reader, TLV::ContextTag(TagTestEFS::kEFS)));

    EXPECT_TRUE(CHIP_NO_ERROR == reader.ExitContainer(outerRead));
    EXPECT_TRUE(*EFS == testSceneEFS);
}
...
TEST_F(TestExtensionFieldSets, TestRemoveExtensionFieldSet)
{
    ...
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->RemoveFieldAtPosition(0));
    EXPECT_TRUE(2 == EFS->GetFieldSetCount());
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS1));
    EXPECT_TRUE(3 == EFS->GetFieldSetCount());
    ...
    // Removal at middle
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->RemoveFieldAtPosition(1));
    EXPECT_TRUE(2 == EFS->GetFieldSetCount());
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS3));
    EXPECT_TRUE(3 == EFS->GetFieldSetCount());
    ...
    // Removal at end
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->RemoveFieldAtPosition(2));
    EXPECT_TRUE(2 == EFS->GetFieldSetCount());
    EXPECT_TRUE(CHIP_NO_ERROR == EFS->InsertFieldSet(EFS3));
    EXPECT_TRUE(3 == EFS->GetFieldSetCount());
