...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionCount)
{
    ...
    EXPECT_TRUE(iterator->Count() == (CHIP_IM_MAX_NUM_SUBSCRIPTIONS / 2));
    ...
    iterator->Release();
    EXPECT_TRUE(count == (CHIP_IM_MAX_NUM_SUBSCRIPTIONS / 2));
...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionMaxCount)
{
    ...
    EXPECT_TRUE(junkBytes.Get() != nullptr);
    EXPECT_TRUE(storage.SyncSetKeyValue(
    ...
    EXPECT_TRUE(err == CHIP_NO_ERROR);
    EXPECT_TRUE(countMax == CHIP_IM_MAX_NUM_SUBSCRIPTIONS);

    // Then check the fake sub is no more
    EXPECT_TRUE(!storage.SyncDoesKeyExist(
        chip::DefaultStorageKeyAllocator::SubscriptionResumption(CHIP_IM_MAX_NUM_SUBSCRIPTIONS + 1).KeyName()));
}
...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionState)
{
    ...
    EXPECT_TRUE(subscriptionInfo == subscriptionInfo1);
    EXPECT_TRUE(iterator->Next(subscriptionInfo));
    EXPECT_TRUE(subscriptionInfo == subscriptionInfo2);
    EXPECT_TRUE(iterator->Next(subscriptionInfo));
    EXPECT_TRUE(subscriptionInfo == subscriptionInfo3);
...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionStateUnexpectedFields)
{
    ...
    EXPECT_TRUE(backingBuffer.Get() != nullptr);
    chip::TLV::ScopedBufferTLVWriter writer(std::move(backingBuffer), subscriptionStorage.TestMaxSubscriptionSize());

    EXPECT_TRUE(subscriptionStorage.TestSave(writer, subscriptionInfo1) == CHIP_NO_ERROR);

    // Additional stuff
    chip::TLV::TLVType containerType;
    EXPECT_TRUE(writer.StartContainer(chip::TLV::AnonymousTag(), chip::TLV::kTLVType_Structure, containerType) == CHIP_NO_ERROR);
    uint32_t value1 = 1;
    uint32_t value2 = 2;
    EXPECT_TRUE(writer.Put(kTestValue1Tag, value1) == CHIP_NO_ERROR);
    EXPECT_TRUE(writer.Put(kTestValue2Tag, value2) == CHIP_NO_ERROR);
    EXPECT_TRUE(writer.EndContainer(containerType) == CHIP_NO_ERROR);

    const auto len = writer.GetLengthWritten();

    writer.Finalize(backingBuffer);

    EXPECT_TRUE(storage.SyncSetKeyValue(chip::DefaultStorageKeyAllocator::SubscriptionResumption(0).KeyName(), backingBuffer.Get(),
                                        static_cast<uint16_t>(len)) == CHIP_NO_ERROR);
...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionStateTooBigToLoad)
{
    ...
    EXPECT_TRUE(additionalBytes.Get() != nullptr);
    EXPECT_TRUE(writer.PutBytes(kTestValue1Tag, additionalBytes.Get(),
...
TEST_F(TestSimpleSubscriptionResumptionStorage, TestSubscriptionStateJunkData)
{
    ...
    junkBytes.Calloc(subscriptionStorage.TestMaxSubscriptionSize() / 2);
    EXPECT_TRUE(junkBytes.Get() != nullptr);
    EXPECT_TRUE(storage.SyncSetKeyValue(chip::DefaultStorageKeyAllocator::SubscriptionResumption(0).KeyName(), junkBytes.Get(),
                                        static_cast<uint16_t>(subscriptionStorage.TestMaxSubscriptionSize() / 2)) == CHIP_NO_ERROR);
