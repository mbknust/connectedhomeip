...
protected:
    ...
    static void NullablesOptionalsEncodeDecodeCheck(bool encodeNulls, bool encodeValues);

    template <typename Encodable, typename Decodable>
    static void NullablesOptionalsEncodeDecodeCheck();

    static void SetupBuf();
    static void DumpBuf();
    static void SetupReader();

    static System::TLVPacketBufferBackingStore store;
    static TLV::TLVWriter writer;
    static TLV::TLVReader reader;
};

System::TLVPacketBufferBackingStore TestDataModelSerialization::store;
TLV::TLVWriter TestDataModelSerialization::writer;
TLV::TLVReader TestDataModelSerialization::reader;

void TestDataModelSerialization::SetUpTestSuite()
{
    VerifyOrDie(chip::Platform::MemoryInit() == CHIP_NO_ERROR);
}

void TestDataModelSerialization::TearDownTestSuite()
{
    {
        System::PacketBufferHandle buf = store.Release();
    }
...
TEST_F(TestDataModelSerialization, EncAndDecSimpleStruct)
{
    ...
    //
    {
        ...
        err = DataModel::Encode(writer, TLV::AnonymousTag(), t);
        EXPECT_TRUE(err == CHIP_NO_ERROR);
    ...
    //
    {
        ...
        err = DataModel::Decode(reader, t);
        EXPECT_TRUE(err == CHIP_NO_ERROR);

        EXPECT_TRUE(t.a == 20);
        EXPECT_TRUE(t.b == true);
        EXPECT_TRUE(t.c == Clusters::UnitTesting::SimpleEnum::kValueA);

        EXPECT_TRUE(t.d.size() == 4);

        for (uint32_t i = 0; i < t.d.size(); i++)
        {
            EXPECT_TRUE(t.d.data()[i] == i);
        }
...
TEST_F(TestDataModelSerialization, EncAndDecSimpleStructNegativeEnum)
{
    ...
    //
    {
        ...
        err = DataModel::Decode(reader, t);
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        EXPECT_TRUE(to_underlying(t.c) == 4);
    }
}
...
TEST_F(TestDataModelSerialization, EncAndDecNestedStruct)
{
    ...
    //
    {
        ...
        EXPECT_TRUE(t.a == 20);
        EXPECT_TRUE(t.b == true);
        EXPECT_TRUE(t.c.a == 11);
        EXPECT_TRUE(t.c.b == true);
        EXPECT_TRUE(t.c.c == Clusters::UnitTesting::SimpleEnum::kValueB);

        EXPECT_TRUE(t.c.d.size() == 4);

        for (uint32_t i = 0; i < t.c.d.size(); i++)
        {
            EXPECT_TRUE(t.c.d.data()[i] == i);
        }
    ...
    //
    {
...

        {
            ...
            while (iter.Next())
            {
                ...
                EXPECT_TRUE(item.a == static_cast<uint8_t>(i));
                EXPECT_TRUE(item.b == true);
                i++;
            }
...

        {
            ...
            {
                auto & item = iter.GetValue();
                EXPECT_TRUE(item == static_cast<uint32_t>(i + 10000));
                i++;
            }
    ...
    //
    {
        ...
        while (iter.Next())
        {
            ...
            while (nestedIter.Next())
            {
                ...
                EXPECT_TRUE(nestedItem.a == (static_cast<uint8_t>(35) + j));
                j++;
            }
...
TEST_F(TestDataModelSerialization, OptionalFields)
{
    ...
    //
    {
        ...
        {
            err = EncodeStruct(writer, TLV::AnonymousTag(),
...
TEST_F(TestDataModelSerialization, InvalidSimpleFieldTypes)
{
    ...
    //
    {
        ...
        //
        {
            ...
            {
                err =
                    EncodeStruct(writer, TLV::AnonymousTag(),
...
void TestDataModelSerialization::NullablesOptionalsEncodeDecodeCheck(bool encodeNulls, bool encodeValues)
{
    ...
    // Encode
    {
        ...
        CHIP_ERROR err = DataModel::Encode(writer, TLV::AnonymousTag(), encodable);
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        err = writer.Finalize();
        EXPECT_TRUE(err == CHIP_NO_ERROR);
    }
    ...
    // Decode
    {
        ...
        Decodable decodable;
        CHIP_ERROR err = DataModel::Decode(reader, decodable);
        EXPECT_TRUE(err == CHIP_NO_ERROR);
        ...
        else if (encodeValues)
        {
            ...
            EXPECT_TRUE(!decodable.nullableInt.IsNull());
            EXPECT_TRUE(decodable.nullableInt.Value() == 5);
            EXPECT_TRUE(decodable.optionalInt.HasValue());
            EXPECT_TRUE(decodable.optionalInt.Value() == 6);
            EXPECT_TRUE(decodable.nullableOptionalInt.HasValue());
            EXPECT_TRUE(!decodable.nullableOptionalInt.Value().IsNull());
            EXPECT_TRUE(decodable.nullableOptionalInt.Value().Value() == 7);
