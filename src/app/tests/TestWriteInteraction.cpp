...
public:
    static void SetUpTestSuite()
    {
        ...
        VerifyOrDie(TestContext::Initialize(&ctx) == SUCCESS);
        ...
        VerifyOrDie(CHIP_NO_ERROR == gGroupsProvider.Init());
        ...
        VerifyOrDie(CHIP_NO_ERROR == ctx.GetBobFabric()->GetCompressedFabricIdBytes(span));
        VerifyOrDie(CHIP_NO_ERROR == chip::GroupTesting::InitData(&gGroupsProvider, ctx.GetBobFabricIndex(), span));
    }
    static void TearDownTestSuite()
    {
        ...
        VerifyOrDie(TestContext::Finalize(&ctx) == SUCCESS);
    }
...
void TestWriteInteraction::GenerateWriteResponse(System::PacketBufferHandle & aPayload)
{
    ...
    EXPECT_TRUE(attributeStatusesBuilder.GetError() == CHIP_NO_ERROR);
    AttributeStatusIB::Builder & attributeStatusIBBuilder = attributeStatusesBuilder.CreateAttributeStatus();
    EXPECT_TRUE(attributeStatusIBBuilder.GetError() == CHIP_NO_ERROR);
...
STATIC_TEST(TestWriteInteraction, TestWriteClientGroup)
{
    ...
    EXPECT_TRUE(writeClient.mState == WriteClient::State::AwaitingDestruction);
}
...
STATIC_TEST(TestWriteInteraction, TestWriteHandler)
{
    ...
    for (auto messageIsTimed : allBooleans)
    {
        for (auto transactionIsTimed : allBooleans)
        {
            ...
            {
                EXPECT_TRUE(status == Status::Success);
            }
            else
            {
                EXPECT_TRUE(status == Status::UnsupportedAccess);
            }
...
STATIC_TEST(TestWriteInteraction, TestWriteRoundtripWithClusterObjects)
{
    ...
    EXPECT_TRUE(callback.mOnSuccessCalled == 0);
    ...
    EXPECT_TRUE(callback.mOnSuccessCalled == 1);

    {
        ...
        reader.Next();
        EXPECT_TRUE(CHIP_NO_ERROR == DataModel::Decode(reader, dataRx));
        EXPECT_TRUE(dataRx.a == dataTx.a);
        EXPECT_TRUE(dataRx.b == dataTx.b);
...
STATIC_TEST(TestWriteInteraction, TestWriteHandlerReceiveInvalidMessage)
{
    ...
    EXPECT_TRUE(InteractionModelEngine::GetInstance()->GetNumActiveWriteHandlers() == 1);
    ...
    EXPECT_TRUE(writeCallback.mLastErrorReason.mStatus == Protocols::InteractionModel::Status::InvalidAction);
    EXPECT_TRUE(InteractionModelEngine::GetInstance()->GetNumActiveWriteHandlers() == 0);
    engine->Shutdown();
...
STATIC_TEST(TestWriteInteraction, TestWriteInvalidMessage1)
{
    ...
    PretendWeGotReplyFromServer(ctx, writeClient.mExchangeCtx.Get());
    ...
    ctx.DrainAndServiceIO();
    EXPECT_TRUE(callback.mError == CHIP_ERROR_INVALID_MESSAGE_TYPE);
...
STATIC_TEST(TestWriteInteraction, TestWriteInvalidMessage2)
{
    ...
    ctx.DrainAndServiceIO();
    EXPECT_TRUE(callback.mError == CHIP_ERROR_END_OF_TLV);
    EXPECT_TRUE(callback.mOnSuccessCalled == 0 && callback.mOnErrorCalled == 1 && callback.mOnDoneCalled == 1);
...
STATIC_TEST(TestWriteInteraction, TestWriteInvalidMessage4)
{
    ...
    ctx.DrainAndServiceIO();
    EXPECT_TRUE(callback.mError == CHIP_IM_GLOBAL_STATUS(Busy));
    EXPECT_TRUE(callback.mOnSuccessCalled == 0 && callback.mOnErrorCalled == 1 && callback.mOnDoneCalled == 1);
