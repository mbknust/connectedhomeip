...
void BuildStatusResponseMessage(chip::TLV::TLVWriter & aWriter)
{
    ...
    EXPECT_TRUE(statusResponse.GetError() == CHIP_NO_ERROR);
}
...
void ParseStatusResponseMessage(chip::TLV::TLVReader & aReader, bool aTestPositiveCase)
{
    ...
    {
        EXPECT_TRUE(status == statusValue && err == CHIP_NO_ERROR);
    }
    else
    {
        EXPECT_TRUE(status != invalidStatusValue && err == CHIP_NO_ERROR);
    }
}
...
TEST_F(TestStatusResponseMessage, StatusResponseMessagePositiveTest)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
...
TEST_F(TestStatusResponseMessage, StatusResponseMessageNegativeTest)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    chip::System::PacketBufferTLVWriter writer;
    chip::System::PacketBufferTLVReader reader;
