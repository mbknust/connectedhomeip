...
TEST_F(TestBuilderParser, ListTest)
{
    ...
    EXPECT_TRUE(listBuilder.Init(&writer) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(listParser.Init(reader) == CHIP_NO_ERROR);
}

TEST_F(TestBuilderParser, StructTest)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    chip::System::PacketBufferTLVWriter writer;
    chip::System::PacketBufferTLVReader reader;
    ...
    EXPECT_TRUE(structBuilder.Init(&writer) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(structParser.Init(reader) == CHIP_NO_ERROR);
}

TEST_F(TestBuilderParser, ArrayTest)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    chip::System::PacketBufferTLVWriter writer;
    chip::System::PacketBufferTLVReader reader;
    ...
    EXPECT_TRUE(arrayBuilder.Init(&writer) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(arrayParser.Init(reader) == CHIP_NO_ERROR);
} // clang-format on
} // namespace