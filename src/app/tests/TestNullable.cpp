...
TEST(TestNullable, TestUpdate)
{
    ...
    // Arrays
    {
        ...
        ASSERT_FALSE(nullable1.IsNull());
        ASSERT_FALSE(nullable2.IsNull());
        EXPECT_EQ(nullable1, nullable2);

        // No-op on change to same.
        EXPECT_EQ(nullable1.Update(nullable2), false);
        EXPECT_EQ(nullable1, nullable2);

        nullable1.Value()[0] = 100;

        EXPECT_NE(nullable1, nullable2);
        EXPECT_EQ(nullable2.Update(nullable1), true);
        EXPECT_EQ(nullable1, nullable2);
    }
    ...
    // Scalar cases
    {
        ...
        EXPECT_EQ(nullable1.Update(nullable1), false);
        ASSERT_FALSE(nullable1.IsNull());

        // Non-null to null
        EXPECT_EQ(nullable1.Update(NullNullable), true);
        EXPECT_TRUE(nullable1.IsNull());

        // Null to null
        EXPECT_EQ(nullable1.Update(NullNullable), false);
        EXPECT_TRUE(nullable1.IsNull());

        // Null to non-null
        EXPECT_EQ(nullable1.Update(MakeNullable(static_cast<uint8_t>(1))), true);
        ASSERT_FALSE(nullable1.IsNull());
        EXPECT_EQ(nullable1.Value(), 1);

        // Non-null to non-null different value
        EXPECT_EQ(nullable1.Update(MakeNullable(static_cast<uint8_t>(2))), true);
        ASSERT_FALSE(nullable1.IsNull());
        EXPECT_EQ(nullable1.Value(), 2);

        // Non-null to extent of range --> changes to "invalid" value in range.
        EXPECT_EQ(nullable1.Update(MakeNullable(static_cast<uint8_t>(255))), true);
        ASSERT_FALSE(nullable1.IsNull());
