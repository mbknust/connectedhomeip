...
TEST(BindingTable, TestEmptyBindingTable)
{
    BindingTable table;
...
TEST(BindingTable, TestAdd)
{
    ...
    EXPECT_TRUE(table.Add(unusedEntry) == CHIP_ERROR_INVALID_ARGUMENT);
    for (uint8_t i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    {
        EXPECT_TRUE(table.Add(EmberBindingTableEntry::ForNode(0, i, 0, 0, NullOptional)) == CHIP_NO_ERROR);
    }
    EXPECT_TRUE(table.Add(EmberBindingTableEntry::ForNode(0, 0, 0, 0, NullOptional)) == CHIP_ERROR_NO_MEMORY);
    EXPECT_TRUE(table.Size() == EMBER_BINDING_TABLE_SIZE);

    auto iter = table.begin();
    for (uint8_t i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    {
        EXPECT_TRUE(iter != table.end());
        EXPECT_TRUE(iter->nodeId == i);
        EXPECT_TRUE(iter.GetIndex() == i);
        ++iter;
    }
    EXPECT_TRUE(iter == table.end());
}

TEST(BindingTable, TestRemoveThenAdd)
{
    BindingTable table;
    chip::TestPersistentStorageDelegate testStorage;
    table.SetPersistentStorage(&testStorage);
    EXPECT_TRUE(table.Add(EmberBindingTableEntry::ForNode(0, 0, 0, 0, NullOptional)) == CHIP_NO_ERROR);
    auto iter = table.begin();
    EXPECT_TRUE(table.RemoveAt(iter) == CHIP_NO_ERROR);
    EXPECT_TRUE(iter == table.end());
    EXPECT_TRUE(table.Size() == 0);
    EXPECT_TRUE(table.begin() == table.end());
    for (uint8_t i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    ...
    ++iter;
    EXPECT_TRUE(table.RemoveAt(iter) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Size() == EMBER_BINDING_TABLE_SIZE - 1);
    EXPECT_TRUE(iter->nodeId == 2);
    EXPECT_TRUE(iter.GetIndex() == 2);
    auto iterCheck = table.begin();
    ++iterCheck;
    EXPECT_TRUE(iter == iterCheck);

    EXPECT_TRUE(table.Add(EmberBindingTableEntry::ForNode(0, 1, 0, 0, NullOptional)) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Size() == EMBER_BINDING_TABLE_SIZE);
    iter = table.begin();
    ...
    EXPECT_TRUE(iter->nodeId == 1);
    EXPECT_TRUE(iter.GetIndex() == 1);
    ++iter;
    EXPECT_TRUE(iter == table.end());
    iter = table.begin();
    EXPECT_TRUE(table.RemoveAt(iter) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Size() == EMBER_BINDING_TABLE_SIZE - 1);
    EXPECT_TRUE(iter == table.begin());
    EXPECT_TRUE(iter.GetIndex() == 2);
    EXPECT_TRUE(iter->nodeId == 2);
    EXPECT_TRUE(table.GetAt(0).type == EMBER_UNUSED_BINDING);
}

void VerifyTableSame(BindingTable & table, const std::vector<EmberBindingTableEntry> & expected)
{
    EXPECT_TRUE(table.Size() == expected.size());
    auto iter1 = table.begin();
    auto iter2 = expected.begin();
    while (iter2 != expected.end())
    {
        EXPECT_TRUE(iter1 != table.end());
...
void VerifyRestored(chip::TestPersistentStorageDelegate & storage, const std::vector<EmberBindingTableEntry> & expected)
{
    ...
    EXPECT_TRUE(restoredTable.LoadFromStorage() == CHIP_NO_ERROR);
    VerifyTableSame(restoredTable, expected);
}

TEST(BindingTable, TestPersistentStorage)
{
    chip::TestPersistentStorageDelegate testStorage;
    BindingTable table;
    ...
    table.SetPersistentStorage(&testStorage);
    EXPECT_TRUE(table.Add(expected[0]) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Add(expected[1]) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Add(expected[2]) == CHIP_NO_ERROR);
    EXPECT_TRUE(table.Add(expected[3]) == CHIP_NO_ERROR);
    ...
    EXPECT_TRUE(table.Add(EmberBindingTableEntry::ForNode(4, 4, 0, 0, NullOptional)) != CHIP_NO_ERROR);
    VerifyRestored(testStorage, expected);
