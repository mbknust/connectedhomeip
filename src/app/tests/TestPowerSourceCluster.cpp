...
// TODO this has duplicate definitions in other files, that all should be moved to AppTestContext.cpp
namespace {
...
{
public:
    static void TestEndpointList(void * apContext);
...
std::vector<EndpointId> ReadEndpointsThroughAttributeReader(EndpointId endpoint)
{
    ...
    EXPECT_TRUE(TagNumFromTag(attrDataReader.GetTag()) == 2);
...
TEST_F(TestPowerSourceCluster, TestEndpointList)
{
    ...
    EXPECT_TRUE(readBack->size() == 5);
    for (size_t i = 0; i < readBack->size(); ++i)
    {
        EXPECT_TRUE(readBack->data()[i] == list0[i]);
    }
    ...
    ASSERT_TRUE(readBack != nullptr);
    EXPECT_TRUE(readBack->size() == 10);
    for (size_t i = 0; i < readBack->size(); ++i)
    {
        EXPECT_TRUE(readBack->data()[i] == list1[i]);
    }
    ...
    for (EndpointId ep = 2; ep < numEndpoints; ++ep)
    {
        ...
        ASSERT_TRUE(readBack != nullptr);
        EXPECT_TRUE(readBack->size() == 1);
        if (readBack->size() == 1)
        {
            EXPECT_TRUE(readBack->data()[0] == listRest[0]);
        }
    }
