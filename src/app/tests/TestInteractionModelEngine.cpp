...
class TestInteractionModelEngine : public chip::Test::AppContext, public ::testing::Test
{
public:
...
TEST_F_FROM_FIXTURE(TestInteractionModelEngine, TestSubjectHasActiveSubscriptionSingleSubOneEntry)
{
    ...
    EXPECT_EQ(engine->SubjectHasActiveSubscription(bobFabricIndex, bobNodeId), false);
...
TEST_F_FROM_FIXTURE(TestInteractionModelEngine, TestSubjectHasActiveSubscriptionMultipleSubsSingleEntry)
{
    ...
    // Verify that both Alice and Bob have no active subscriptions
    EXPECT_EQ(engine->SubjectHasActiveSubscription(bobFabricIndex, bobNodeId), false);
    EXPECT_EQ(engine->SubjectHasActiveSubscription(aliceFabricIndex, aliceNodeId), false);
...
TEST_F_FROM_FIXTURE(TestInteractionModelEngine, TestSubjectHasActiveSubscriptionSubWithCAT)
{
    ...
    EXPECT_EQ(engine->SubjectHasActiveSubscription(bobFabricIndex, valideSubjectId), false);
    EXPECT_EQ(engine->SubjectHasActiveSubscription(bobFabricIndex, invalideSubjectId), false);
...
TEST_F(TestInteractionModelEngine, TestSubjectHasPersistedSubscription)
{
    ...
    EXPECT_EQ(InteractionModelEngine::GetInstance()->SubjectHasPersistedSubscription(fabric1, nodeId1), false);
    ...
    EXPECT_EQ(InteractionModelEngine::GetInstance()->SubjectHasPersistedSubscription(fabric2, nodeId2), false);
...
TEST_F_FROM_FIXTURE(TestInteractionModelEngine, TestSubscriptionResumptionTimer)
{
    ...
    EXPECT_EQ(timeTillNextResubscriptionMs, CHIP_CONFIG_SUBSCRIPTION_TIMEOUT_RESUMPTION_MIN_RETRY_INTERVAL_SECS);
         ...
         InteractionModelEngine::GetInstance()->mNumSubscriptionResumptionRetries++)
    {
        ...
        EXPECT_LT(timeTillNextResubscriptionMs, CHIP_CONFIG_SUBSCRIPTION_TIMEOUT_RESUMPTION_MAX_RETRY_INTERVAL_SECS);
    ...
    timeTillNextResubscriptionMs = InteractionModelEngine::GetInstance()->ComputeTimeSecondsTillNextSubscriptionResumption();
    EXPECT_EQ(timeTillNextResubscriptionMs, CHIP_CONFIG_SUBSCRIPTION_TIMEOUT_RESUMPTION_MAX_RETRY_INTERVAL_SECS);
}
...
TEST_F_FROM_FIXTURE(TestInteractionModelEngine, TestDecrementNumSubscriptionsToResume)
{
    ...
    err = engine->Init(&GetExchangeManager(), &GetFabricTable(), app::reporting::GetDefaultReportScheduler());
    EXPECT_EQ(err, CHIP_NO_ERROR);
